ad_page_contract {
    ...
    @cvs-id $Id: index.tcl,v 1.2 2014/03/25 14:58:40 nsadmin Exp $

    USER   DATA       MODIFICHE
    ====== ========== =======================================================================
    nic01  25/03/2014 Visto che il rendi amministratore non funziona, come chiesto da Luca, lo
    nic01             elimino.
...
if { $admin_p || [parameter::get -parameter "MembersCanInviteMembersP" -default 0] } {
    set actions [list "Aggiungi un utente" user-new?email= "Aggiungi un utente"]
}
...
template::list::create \
    ...
    -elements {
	    ...
	    link_url_col member_url 
            link_html {title "User Home page"}
        }
        email {
...
db_multirow -extend { 
    ...
    if { $admin_p } {
        set member_url [export_vars -base /acs-admin/users/one { user_id }]
        switch $member_state {
            approved {
                if { $member_admin_p == 0 } {
                    #nic01 set make_admin_url [export_vars -base make-admin { user_id }]
		    set make_admin_url "";#nic01
                } else {
                    #nic01 set make_member_url [export_vars -base make-member { user_id }]
		    set make_member_url "";#nic01
                }
                if { $main_site_p } {
