...
set titolo       "Filtro Ristampa Fatture Ispezione"
set button_label "Seleziona" 
set page_title   "Filtro Ristampa Fatture Ispezione"
...
element create $form_name anno_fatt \
    -label anno_fatt\
    -widget   text \
    -datatype text \
...
if {[form is_request $form_name]} {
    ...
    if {[string equal $anno_fatt ""]} {
        set anno_fatt [string range [iter_set_sysdate] 0 3]
    }
    element set_properties $form_name anno_fatt        -value $anno_fatt
}
...
if {[form is_valid $form_name]} {
    ...
    set anno_fatt          [element::get_value $form_name anno_fatt]
    set error_num 0
    ...
    if {[string equal $anno_fatt ""]} {
        element::set_error $form_name anno_fatt "Inserire l'anno"
        incr error_num
    } else {
        if {![string is integer $anno_fatt]} {
            element::set_error $form_name anno_fatt "Deve essere un numero intero"
            incr error_num
        } else {
            if {[string length $anno_fatt] == 2} {
                element::set_error $form_name anno_fatt "Deve essere nel formato di quattro numeri"
                incr error_num
            }
        }
    }
    ...
    set link_gest [export_url_vars f_da_num_fatt f_a_num_fatt nome_funz nome_funz_caller anno_fatt]


    #set return_url "coimfatt-list?$link_gest"    
    set return_url "coimfatt-isp-layout-multi?$link_gest"    
