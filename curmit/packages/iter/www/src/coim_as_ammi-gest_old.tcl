...
&& [string equal $cod_impianto ""]
    ...
    set cerca_viae [iter_search $form_name [ad_conn package_url]/tabgen/coimviae-filter [list dummy f_cod_via dummy indirizzo dummy toponimo cod_comune cod_comune dummy dummy]]
} else {
    set cerca_viae ""
}
...
if {[form is_request $form_name]} {
    ...
    if {$funzione == "I"} {
		...
		set conta 0
		db_foreach sel_gen_aimp2 "select gen_prog , potenza , cod_combustibile
	                               from coimgend_as_resp
	                              where cod_as_resp = :cod_as_resp
	                           order by gen_prog" {
				       incr conta 
  ...
  # form valido dal punto di vista del templating system
    ...
    if {![string equal $cod_impianto ""]} {
		...
		db_foreach sel_gen_aimp "select gen_prog as gen_prog_g,
	                                    pot_focolare_nom as pot_focolare_nom_g,
		    ...
		    set cod_combustibile($conta) $cod_combustibile_g
			 set potenza_gend_chek  [iter_check_num $potenza_gend($conta) 2]
			set potenza_gend_tot		 [expr $potenza_gend_tot +   $potenza_gend_chek]
  ...
  # controlli standard su numeri e date, per Ins ed Upd
	...
	if {[string equal $cod_impianto ""]} {
		...
		if {[string equal $potenza_gend($conta) ""]} {
		    ...
		    set potenza_gend($conta) [iter_check_num $potenza_gend($conta) 2]
		    if {$potenza_gend($conta) == "Error"} {
			element::set_error $form_name potenza_gend.$conta "Deve essere numerico e pu&ograve; avere al massimo 2 decimali"
			incr error_num
		    } else {
			if {[iter_set_double $potenza_gend($conta)] >=  [expr pow(10,7)]
			    ||  [iter_set_double $potenza_gend($conta)] <= -[expr pow(10,7)]} {
    ...
    if {[info exists dml_sql]} {
		...
		if {[info exists dml_sql_gend]} {
		    ...
		    while {$conta < $conta_max} {
				...
				set potenza_db  $potenza_gend($conta)
