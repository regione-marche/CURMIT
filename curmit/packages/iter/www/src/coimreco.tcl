...
set page_title   "Stampa registro corrispettivi"
...
if {[form is_valid parameters]} {
    ...
    set nome_file        "stampa registro corrispettivi"
...
#ns_return 200 text/html "select distinct perc_iva::integer from coimrfis where to_char(data_rfis, 'YYYY-MM-DD') between '${year_and_month}-01' and '${year_and_month}-31'"; return
 ...
 "} else {
    ...
    db_foreach query "
        select  to_char(data_rfis, 'DD/MM/YYYY') as data_rfis_pretty
              , data_rfis
              ...
              , iter_edit_num((sum(coalesce(imponibile,0)) + sum(coalesce(spe_legali,0)) + sum(coalesce(spe_postali,0))),2) as tota_pretty
              , sum(imponibile) as tota_imp
              , sum(coalesce(spe_postali,0)) as tota_post
              , sum(coalesce(spe_legali,0)) as tota_leg
              , sum(coalesce(imponibile,0)) + sum(coalesce(spe_legali,0)) + sum(coalesce(spe_postali,0)) as tota
        from coimrfis 
        where to_char(data_rfis, 'YYYY-MM-DD') between '${year_and_month}-01' and '${year_and_month}-31'
              and flag_pag = :flag_pag
        group by data_rfis        
        order by data_rfis
    " {
       set lista_num [db_list query "select distinct(num_rfis) from coimrfis where to_char(data_rfis, 'YYYY-MM-DD') between '${year_and_month}-01' and '${year_and_month}-31'
              and coalesce(flag_pag, 'N') = :flag_pag  and data_rfis = :data_rfis"]
       set lista_pag [db_list query "select distinct(estr_pag)  from coimrfis where to_char(data_rfis, 'YYYY-MM-DD') between '${year_and_month}-01' and '${year_and_month}-31'
              and coalesce(flag_pag, 'N') = :flag_pag  and data_rfis = :data_rfis and estr_pag is not null"]
#scorporo
