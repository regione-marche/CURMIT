ad_page_contract {
    ...
    USER  DATA       MODIFICHE
    ===== ========== =======================================================================
    sim01 06/07/2017 Corretto selezione del cimp della regione toscana
...
if {$flag_tracciato eq "RE"} {#sim01 if e suo contenuto
    if {$coimtgen(ente) eq "PLI"} {
	set gest_prog       "../srcpers/PLI/coimcimp-re-gest"
    } else {
	set gest_prog       "coimcimp-re-gest"
    }
} else {#sim01 
...
#if {$caller == "index"} {
    set link    "\[export_url_vars flag_tracciato cod_impianto gen_prog last_cod_cimp nome_funz nome_funz_caller extra_par caller url_aimp url_list_aimp flag_cimp flag_inco extra_par_inco cod_inco \]"
    set actions "
    <td nowrap><a href=\"$gest_prog?funzione=I&$link\">Selez.</a></td>"
    set js_function ""
#} else { 
