ad_page_contract {
    ...
    @cvs-id coimestr-mail-list.tcl

    USER  DATA       MODIFICHE
    ===== ========== =======================================================================
    sim02 16/03/2021 Il tipo estrazione 1 deve sempre considerate la potenza <100 e non <35.

    sim01 16/03/2021 La query non deve tenere contro delle DAM ma solo dei rapporti di ispezione
    sim01            come già avviene per coimestr-list.tcl

    rom03 17/09/2018 Vengono estratti solamente gli impianti che hanno l'ultima data controllo
    rom03            della coimdimp maggiore o uguale al '2017-01-01'.

    rom02 16/07/2018 Aggiunte colonne Cod.Manu e Manutentore alla lista su richiesta di Sandro
...
set gest_prog       "coimestr-mail-gest"
set list_prog       "coimestr-mail-list"
...
#rom02 Aggiunte colonne Cod.Manu e Manutentore su richiesta di Sandro
...
if {$tipo_estrazione eq "1" || $tipo_estrazione eq "3" || $tipo_estrazione eq "9" || $tipo_estrazione eq "13" || $tipo_estrazione eq "14" || $tipo_estrazione eq "11" || $tipo_estrazione eq "12" || $tipo_estrazione eq "15" || $tipo_estrazione eq "16" || $tipo_estrazione eq "17" || $tipo_estrazione eq "18"} {
    set table_def [list \
		       ...
		       [list cod_manutentore  "Cod.Manu"     no_sort      {c}] \
   ...
   set table_def [list \
		       ...
		       [list cod_impianto_est "Codice"         no_sort      {l}] \
		       [list nome_resp        "Responsabile"   no_sort      {l}] \
                       [list cod_manutentore  "Cod.Manu"       no_sort      {l}] \
                       [list manutentore      "Manutentore"    no_sort      {l}] \
		       [list comune           "Comune"         no_sort      {l}] \
		       [list via              "Indirizzo"      no_sort      {l}] \
		       [list flag_tipo_impianto  "TI"          no_sort  $stato_imp] \
...
switch $tipo_estrazione {
	...
	set where_pote " and a.potenza < 100";#sim02
...
#fine
	...
	#rom03 Vado in join anche nell'estrazione "Globale" per fare il controllo sulla data_controllo della coimdimp
	set from_dimp " , coimdimp e";#rom03
        set where_dimp " and e.cod_impianto = a.cod_impianto
                         and e.data_controllo = (select max(f.data_controllo)
                                                   from coimdimp f
                                                  where f.cod_impianto = a.cod_impianto
...
append where_dimp " and e.data_controllo >= '2017-01-01'";#rom03
