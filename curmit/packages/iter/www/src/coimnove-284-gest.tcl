ad_page_contract {
    ...
    USER  DATA       MODIFICHE
    ===== ========== ==========================================================================
    rom01 08/02/2021 Corretto errore che faceva inseire in modo errato il codice cittadino.
...
element create $form_name flag_dichiarante \
    ...
    -optional \
    -options {{"Legale rappresentante" "L"} {"Responsabile tecnico" "R"} {"Tecnico specializzato" "T"} {"Responsabile dell'impianto" "I"}}
...
if {$funzione == "I" || $funzione == "M"} {
    ...
    set cerca_citt [iter_search $form_name coimcitt-filter [list dummy cod_cittadino f_cognome cognome  f_nome nome ]]
} else {
    set cerca_manu ""
    set cerca_citt ""
}
...
if {[form is_request $form_name]} {
	...
	ad_returnredirect [ad_conn package_url]src/coimaimp-warning?cod_impianto=$cod_impianto&redirect=coimnove-284-gest&$link&funzione=$funzione&caller=$caller
	ad_script_abort    
    }
...
if {[form is_valid $form_name]} {
    ...
    set cod_cittadino          [element::get_value $form_name cod_cittadino]
	...
	set cod_manutentore ""
	set cognome_dichiarante ""
	set nome_dichiarante ""
    }
    if {$cognome eq "" && $nome eq ""} {
	set cod_cittadino ""
    }
	...
	if {$verif_emis_286_no == "t"} {
        ...
        set check_cod_citt {
            ...
            switch $ctr_citt {
                0 { set chk_out_msg "Soggetto non trovato"}
                1 { #rom01 set chk_out_cod_citt $cod_manutentore
		    set chk_out_cod_citt $cod_cittadino;#rom01
                    set chk_out_rc       1 }
                default {
                    if {$chk_out_rc == 0} {
                        set chk_out_msg "Trovati pi&ugrave; soggetti: usa il link cerca"
	...
	if {$cod_manutentore ne "" && $cod_cittadino ne ""} {
	    element::set_error $form_name cognome_manu "Inserire solo uno tra installatore/manutentore e soggetto responsabile"
	    incr error_num
	} else {
	
	    if {[string equal $cognome_manu ""] && [string equal $nome_manu ""] && [string equal $cognome ""] && [string equal $nome ""]} {
		
		element::set_error $form_name cognome_manu "Inserire installatore/manutentore o soggetto responsabile"
		incr error_num
		...
		if {$flag_installatore eq "" && $flag_rispetta_val_min eq "" && $flag_manutentore eq "" && $cod_manutentore ne ""} {
		...
		if {[string range $cod_manutentore 0 1] == "MA"} {
		    ...
		    set chk_inp_cod_citt $cod_cittadino
...
#    }
    ...
    if {$flag_dich_conformita eq "t" && $dich_conformita_nr eq ""} {
	element::set_error $form_name dich_conformita_nr "Inserire numero"
	incr error_num
    }
    ...
    set link_gest      [export_url_vars cod_impianto cod_noveb nome_funz nome_funz_caller url_list_aimp url_aimp extra_par caller]
    switch $funzione {
        M {set return_url   "coimnove-284-gest?funzione=V&$link_gest"}
        D {set return_url   "coimdimp-list?$link_list"}
        I {set return_url   "coimnove-284-gest?funzione=V&$link_gest"}
