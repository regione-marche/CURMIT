...
set nom       "Caricam. modelli manutentori"
...
iter_get_coimtgen
	...
	if {![string equal $cod_manu ""]} {
	    set flag_manu "T"
	    set link_saldo "<a href=coimcari-manu-gest?flag_saldo=T&nome_funz=cari-manu>Visualizza il saldo residuo</a>"
	    if {$flag_saldo == "T"} {
		...
		append saldo " &#8364;"
		set conto_manu "\n Codice portafoglio: "
		append conto_manu [string range $parte_2 [expr [string first " " $parte_2] + 1] end]
	    } else {
		set saldo ""
		set conto_manu ""
	    }
...
set titolo              "Caricam. modelli manutentori"
switch $funzione {
...
if {[form is_valid $form_name]} {
  # form valido dal punto di vista del templating system
	...
	#decido quale tracciato richiamare
	if {[catch {set file_id [open $file_name r]}]} {
	    iter_return_complaint "File csv di input non aperto: $file_name"
	}
	fconfigure $file_id -encoding iso8859-1
	iter_get_csv $file_id file_inp_col_list |
	iter_get_csv $file_id file_inp_col_list |
	set lunghezza_letta [llength $file_inp_col_list]
	close $file_id
	if {$lunghezza_letta == "148"} {
	    set nom_prog "iter_cari_manu"
	} elseif { $lunghezza_letta == "181" || $lunghezza_letta == "187"} {
	    set nom_prog "iter_cari_manu_new"
	} else {
	    element::set_error $form_name file_name "Il numero dei campi letti non corrisponde a nessun tracciato"
	    incr error_num
	    ad_return_template
	    return
	}
...
#        lappend par        "S"
  ...
  # Lancio la query di manipolazione dati contenute in dml_sql
    ...
    switch $funzione {
        I {set return_url   "coimcari-manu-gest?funzione=V&$link_gest"}
        V {set return_url   ""}
    }
