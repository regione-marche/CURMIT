...
switch $nome_funz {
    "tele"  {set page_title "Inserimento allegato E3 - Bonifica impianti"
	set titolo          "Inserimento allegato E3 - Ricerca Impianti"     
    }
    default {set page_title "Inserimento allegato E3 - Bonifica impianti"
	set titolo          "Inserimento allegato E3 - Bonifica impianti"
    }
}
...
if {$flag_viario == "T"} {
	...
	if {$cod_comune_chk eq ""} {
    	#ns_return 200 text/html "qua";return
...
if {[form is_valid $form_name]} {
  ...
  # per il Comune    e' necessario sapere se e' indicato il quartiere o la via
	...
	if {[string equal $cod_impianto_est_new ""]} {
	    append mex_err_cod_impianto "Inserire il codice bollino"
	} else {
	    if {[string length $cod_impianto_est_new] < 6} {
		append mex_err_cod_impianto "Il codice bollino deve essere lungo almeno 6 caratteri" 
	    }
	}
    }
  ...
  # in questo modo si e' sicuri che f_comune e' stato valorizzato.
    ...
    set return_url "coimtele-list?$link_list"    
