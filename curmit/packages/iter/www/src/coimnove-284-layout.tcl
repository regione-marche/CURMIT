...
if {[db_0or1row $sel_aimp_2 ""] == 0} {
	...
	if {[string range $cod_manutentore 0 1] == "MA"} {
	    ...
	    set is_manutentore "t"
	} else {
	    db_0or1row sel_dati_citt ""
	    set is_manutentore "f"
	    set flag_a  ""
	    set flag_c  ""
	    set flag_e  ""
	}
...
"
	...
	if {$is_manutentore eq "t"} {
	    ...
	    #in questo caso è stato scelto un cittadino
	    append    stampa2 "
            <tr><td align=left>Il sottoscritto $cognome_manu $nome_manu in qualità di <b>responsabile dell'impianto</b> di cui sopra, dichiara che lo stesso è conforme alle caratteristiche tecniche di cui all'art. 285 ed è idoneo a rispettare i valori di cui all'art. 286 del D.Lgs. 152/06.</td></tr>"
