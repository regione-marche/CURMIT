ad_page_contract {
    ...
    USER  DATA       MODIFICHE
    ===== ========== =======================================================================
    sim40 30/09/2020 Su richiesta della Regione Marche è stato cambiato il messaggio di errore
    sim40            in caso di impianto non attivo. Va bene per tutti gli enti.

    sim39 08/05/2017 Firenze può avere sia RCT che 1B quindi gestico il doppio flag_tracciato
...
#sim40        Funzione impossibile per impianti annullati/Non attivi/Rottamati"  
	iter_return_complaint "Sarà possibile trasmettere RCEE e altri moduli solo dopo la riattivazione/validazione dell'impianto da parte dell'ente";#sim40
	ad_script_abort
    }
}
#fine modifica
...
if {[form is_valid $form_name]} {
    ...
    if {$id_tplg ne "9" && $note_tplg_altro ne ""} {
	 element::set_error $form_name note_tplg_altro "Scrivere la nota solo se è stata selezionata la tipologia altro"
         incr error_num
    }
    if {$id_tplg eq "9" && $note_tplg_altro eq ""} {
         element::set_error $form_name note_tplg_altro "E' obbligatorio specificare il campo note se è stata selezionata la tipologia altro"
         incr error_num
    }
