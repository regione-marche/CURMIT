ad_page_contract {
    ...
    @cvs-id          coimdimp-g-gest.tcl

    USER  DATA       MODIFICHE
    ===== ========== =======================================================================
    sim07 30/09/2020 Su richiesta della Regione Marche è stato cambiato il messaggio di errore
    sim07            in caso di impianto non attivo. Va bene per tutti gli enti.

    sim06 21/03/2017 Riportato correzione già fatta da Nicola sul programma degli rct

    sim05 17/11/2016 Gestito la potenza in base al flag_tipo_impianto

    sim04 04/10/2016 Su richiesta di Sandro, per Taranto, non vanno fatti i controlli tra
    sim04            Data controllo e data inserimento, il controllo di obbligatorietà dell'operatore
    sim04            e in locale di installazione va messo non noto

    sim03 06/12/2016 Corretto errore in fase di modifica

    sim02 27/06/2016 Se flag_tariffa_impianti_vecchi eq "t" e il combustibile e' Gas o Metano
    sim02            devo verificare se l'impianto e' vecchio e quindi usare un'altra tariffa.

    nic02 09/06/2016 Sandro conferma che in generale, i modelli G non devono essere agganciati
    nic02            al wallet perchè ormai sono obsoleti.
...
set controllo_tariffa {#sim02
    
    set cod_listino 0
    if {[db_0or1row sel_tari ""] == 0} {
	set tariffa ""
	set flag_tariffa_impianti_vecchi "";#sim02
	set anni_fine_tariffa_base       "";#sim02
	set tariffa_impianti_vecchi      "";#sim02
    }
    
    #il controllo sugli anni va fatto solo se l'apposito flag è true e solo per GAS e METANO
    if {$flag_tariffa_impianti_vecchi eq "t" && ($combustibile eq "4" || $combustibile eq "5")} {#sim02: aggiunta if e suo contenuto
	set data_insta_controllo [db_string q "select coalesce(:data_insta::date,'1900-01-01')"]    
	ns_log notice "coimdimp-g-gest;data_insta_controllo:$data_insta_controllo"
...
if {$funzione eq "I"} {
    ...
    if {$stato_imp ne "A" } {
#sim07        iter_return_complaint "
#sim07        Funzione impossibile per impianti annullati/Non attivi/Rottamati"  
	iter_return_complaint "Sarà possibile trasmettere RCEE e altri moduli solo dopo la riattivazione/validazione dell'impianto da parte dell'ente";#sim07
	ad_script_abort
    }
}
#fine modifica
...
if {$funzione ne "I"} {
    db_1row query "select coalesce(stato_dich, '') as stato_dich from coimdimp where cod_dimp = :cod_dimp"    
...
if {$funzione != "I"} {
    ...
    set cod_potenza_old $cod_potenza;#sim03
...
if {$funzione == "I" || $funzione == "M"} {
    ...
    #		    set link_gend "flag_tracciato=G&[export_url_vars cod_impianto last_cod_cimp nome_funz nome_funz_caller tabella cod_dimp extra_par caller url_aimp url_list_aimp]"
...
if {[form is_request $form_name]} {
	...
	#sim02	if {[db_0or1row sel_tari ""] == 0} {
	#sim02	    set tariffa ""
	#sim02	}
	
	eval $controllo_tariffa;#sim02
	...
	set nome_utente_ins ""
        set data_ins_edit ""
	if {$utente_ins ne ""} {
	    #Messa db_0or1row perche' per Rimini non veniva trovato l'utente
	    #Se non viene trovato l'utente, nome_utente_ins e' gia' impostato a null
	    db_0or1row sel_utente_ins ""
	}
	if {$data_ins ne ""} {
...
if {[form is_valid $form_name]} {
	...
	if {$cod_opmanu_new eq ""} {
	    ...
	    # Publies (PPO e iterprfi_pu) l'ha chiesto il 01/09/2014
            # La provincia di Cosenza l'ha chiesto il 02/08/2016
	    # sim04 Taranto l'ha chiesto il 04/10/2016
	    if {$coimtgen(ente) ne "PLI"
	    &&  $coimtgen(ente) ne "PVE"
	    &&  $coimtgen(ente) ne "PCE"
	    &&  $coimtgen(ente) ne "PPO"
            &&  $coimtgen(ente) ne "PCS"
	    &&  ![string match "*iterprfi_pu*" [db_get_database]]
	    &&  $coimtgen(ente) ne "PPD"
	    &&  $coimtgen(ente) ne "PTA" 	
	    } {#02/08/2016, Sandro: aggiunta if (prima non c'era)
      ...
      if {[string equal $cognome_resp ""] &&  [string equal $nome_resp ""]} {
             element::set_error $form_name cognome_resp "Responsabile : dato obbligatorio"
                incr error_num
	}
	...
	if {$coimtgen(ente) ne "PTA"} {;#sim04

	#sf 211014
        #sf spostata data dal 16 10 2014 al 01 05 2016 il  02 08 2016
	if {$data_controllo >= "20160501"} {
	    set flag_errore_data_controllo "t"
	...
	if {$flag_errore_data_controllo == "f" && $data_controllo >= "20090801"} {
		...
		if {$fisc_r eq ""} {
		    element::set_error $form_name cognome_resp "Il responsabile non ha il Codice Fiscale: Ã¨ obbligatorio per i modelli con data controllo superiore al 31 Luglio 2009"
		    incr error_num
		}
	    }
	}
	...
	#sim06: Nicola 23/06/2014: se $flag_mod_gend == "N" devo valoriz. la var. data_insta
	...
	#sandro 230513
	if {$potenza > $pot_focolare_nom } {
	    element::set_error $form_name potenza "Potenza focolare utile > della nominale"
	    incr error_num
	}
	#sandro 120913
	if {[string equal $potenza ""] || $potenza <= 0.0} {
	    element::set_error $form_name potenza "Potenza focolare utile > 0"
	    incr error_num
	}
	#fine
	...
	if {![string equal $data_scad_pagamento ""]} {
	    ...
	    if {[string equal $data_scadenza_autocert ""]} {
		if {$sw_data_controllo_ok == "t"} {
		    set data_scadenza_autocert [db_string query "select :data_controllo::date + 775" -default ""]
		}
	    } else {
		set data_scadenza_autocert [iter_check_date $data_scadenza_autocert]
		if {$data_scadenza_autocert == 0} {
    ...
    eval $controllo_tariffa;#sim02
    set  costo [iter_check_num $tariffa 2];#sim02
    element set_properties $form_name costo -value $tariffa;#sim02
