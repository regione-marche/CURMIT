ad_page_contract {
    ...
    gab02 08/02/2017 Personaliz. per Comune di Ancona: ricodificare gli impianti come da Legge
    gab02            Reg. Marche CMAN.
    ...
    nic01            questa dicitura: "Bonificato il dd/mm/yyyy; codice dell'impianto di
    nic01            destinazione: cod_impianto_est"
    nic01            Mi appunto che non sono riuscito a provare la bonifica perchè il
    nic01            parametro compatta_list arriva vuoto anche se non ho capito il perchè
    nic01            guardando i sorgenti. Sandro ha detto che ad un certo punto era stato
    nic01            inibito volutamente.
...
set form_valid {
	...
	set cod_as_resp           [element::get_value $form_name cod_as_resp]
   	db_transaction {
		...
		db_dml query "update coim_as_resp
                                 set cod_impianto = :cod_impianto_new
                               where cod_as_resp = :cod_as_resp"

		db_1row query "select cod_legale_rapp
                                    , flag_tracciato
                                    , flag_ammimp
                                 from coim_as_resp
                                where cod_as_resp = :cod_as_resp"

		db_1row query "select flag_resp
                                 from coimaimp
		...
		if {$flag_tracciato == "LMANU"} {
		    if {$flag_ammimp == "R"} {
			set flag_resp "A"
			db_dml query "update coimaimp
                                         set cod_responsabile   = :cod_legale_rapp
                                           , cod_amministratore = :cod_legale_rapp
                                           , flag_resp          = :flag_resp
                                       where cod_impianto = :cod_impianto_new"
		    } else {
			db_dml query "update coimaimp
                                         set cod_amministratore = :cod_legale_rapp
                                       where cod_impianto = :cod_impianto_new"
			...
			if {$flag_resp != "T"} {
			    db_dml query "insert
                                        ...
                                        values ( nextval('coimtodo_s')
                                               , :cod_impianto_new
                                               , ' '
                                               , 4
                                               ,'Presente amministratore non responsabile impianto'
                                               , current_date)"
			}
		    }
		} else {
    ...
    if {![string equal $cod_as_resp ""]} {
	set return_url "coim_as_resp-gest?funzione=V&[export_url_vars flag_tracciato cod_as_resp nome_funz_caller]&nome_funz=[iter_get_nomefunz coim_as_resp-gest]&cod_impianto=$cod_impianto_new"
    } else {
	if {$flag_tracciato == "HMANU"
	||  $flag_tracciato == "IMANU"
	||  $flag_tracciato == "LMANU"} {
	    set return_url "coim_as_resp-gest?funzione=I&[export_url_vars flag_tracciato cod_as_resp nome_funz_caller]&nome_funz=[iter_get_nomefunz coim_as_resp-gest]&cod_impianto=$cod_impianto_new"
	} else {
	    set return_url "coimdimp-gest?funzione=I&[export_url_vars flag_tracciato cod_as_resp nome_funz_caller search_word cod_impianto_est_new]&nome_funz=[iter_get_nomefunz coimdimp-gest]&flag_no_link=T&cod_impianto=$cod_impianto_new"
	}
    }
