...
if {$coimtgen(ente) eq "PUD" || $coimtgen(ente) eq "PGO" || $coimtgen(ente) eq "CCARRARA"} { 
...
if {$coimtgen(regione) eq "CAMPANIA"} {#rom08 if ma non contenuto
    db_1row query "
...
#	}
	...
	if {$coimtgen(regione) ne "MARCHE"} {
		...
		append elenco_anomalie "<tr><td>Inserire il campo <b>Potenza frigorifera: utile (kW)</b> su Scheda 4: Generatori</td></tr>"
	    }
	    if {[string is space $pot_utile_nom]} {
		append elenco_anomalie "<tr><td>Inserire il campo <b>Potenza frigorifera: assorb.(kW)</b> su Scheda 4: Generatori</td></tr>"
	    }
	    if {[string is space $pot_focolare_nom]} {
		append elenco_anomalie "<tr><td>Inserire il campo <b>Potenza frigorifera: nominale (kW)</b> su Scheda 4: Generatori</td></tr>" 
	    }
	} else {
...
#gac03    }
	...
	if {[string is space $cod_tpim] && $flag_tipo_impianto ne "F"} {
	    append elenco_anomalie "<tr><td>Inserire <b>Tipologia impianto</b> su Scheda 1Bis: Dati Generali</td></tr>"
	}
	if {[string is space $unita_immobiliari_servite] && $flag_tipo_impianto ne "F"} {
