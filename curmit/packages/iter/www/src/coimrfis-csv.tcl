...
if {![string equal $da_data ""]} {
    set where_da_data "and to_char(data_rfis,'yyyymmdd') >= :da_data"
} else {
    set where_da_data ""
}

if {![string equal $a_data ""]} {
    set where_a_data  "and to_char(data_rfis,'yyyymmdd') <= :a_data"
} else {
    set where_a_data  ""
}

set current_datetime [clock format [clock seconds] -f "%Y-%m-%d %H:%M:%S"]
set nome_file          "Scarico Elenco ricevute"
...
set     head_cols ""
lappend head_cols "Numero_ricevuta"
lappend head_cols "Data_ricevuta"
...
set     file_cols ""
lappend file_cols "num_rfis"
lappend file_cols "rfis_dt"
