    ...
    <fullquery name="sel_dimp_si_vie">
       <querytext>
		       ...
		       ,e.numero    as numero_util
                       ...
                       ,j.lg_flag_dep_combust_solido
                       ,j.lg_flag_puliz_camino
                       ,j.lg_flag_sep_gen
                       ,j.lg_flag_sollecit_termiche
                       ,t.descrizione as tipologia
                       ,j.note_tplg_altro
                       , case j.lg_condensazione
           	           when 'P' then 'Presente'
                           else '&nbsp;'
                         end as lg_condensazione
                       , case j.lg_vaso_espa
                           when 'A' then 'Aperto'
                           when 'C' then 'Chiuso'
                           else '&nbsp;'
                         end as lg_vaso_espa
                       , case j.lg_marcatura_ce
                           when 'P' then 'Presente'
                           when 'A' then 'Assente'
                           else '&nbsp;'
                         end as lg_marcatura_ce
                       , case j.lg_placca_cam
                           when 'P' then 'Presente'
                           when 'A' then 'Assente'
                           else '&nbsp;'
                         end as lg_placca_cam
                       , case j.lg_caric_comb
                           when 'A' then 'Automatica'
                           when 'M' then 'Manuale'
                           else '&nbsp;'
                         end as lg_caric_comb
                       , case j.lg_mod_evac_fumi
                           when 'N' then 'Naturale'
                           when 'F' then 'Forzata'
                           else '&nbsp;'
                         end as lg_mod_evac_fumi
                       , case j.lg_aria_comburente
                           when 'E' then 'Da esterno'
                           when 'I' then 'Da locale istallazione'
                           else '&nbsp;'
                         end as lg_aria_comburente                       
                       , j.libretto_manutenz
                       , j.doc_prev_incendi 
                       , j.dich_152_presente
                       , j.doc_ispesl       
                  ...
                  left outer join coimtplg t on t.id_tplg        = j.id_tplg
