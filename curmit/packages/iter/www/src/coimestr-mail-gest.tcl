ad_page_contract {
    ...
    @cvs-id          coimestr-mail-gest.tcl

    USER   DATA       MODIFICHE
    ====== ========== =======================================================================
    rom02 16/07/2018  Aggiunto controllo sul manutentore: se vengono selezionati manutentori
    rom02             diversi blocco l'utente.
...
set primo_cod_manu "";#rom02
set ctr_cod_manu   0;#rom02
...
if {[exists_and_not_null conferma]} {
	...
	#nic01	if {[db_0or1row sel_inco ""] == 0}
		...
		|| [db_0or1row sel_inco ""] == 0
		    ...
		    set cod_manu [db_string q "select cod_manutentore from coimaimp where cod_impianto = :cod_impianto"];#rom02
		    incr ctr_cod_manu ;#rom02
		    if {$ctr_cod_manu == "1"} {#rom02 if else e loro contenuto
			set primo_cod_manu $cod_manu
			set codice_mantentore ""
		    } elseif { $ctr_cod_manu > "1"} {
			set codice_mantentore $cod_manu
		    }
		    if {![string equal $codice_mantentore ""] && ($primo_cod_manu ne $codice_mantentore)} {#rom02
			iter_return_complaint "Sono stati selezionati impianti di manutentori diversi, impossibile procedere"
		    }
		}
	    }
	}
