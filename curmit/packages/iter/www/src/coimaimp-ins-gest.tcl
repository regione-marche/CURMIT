ad_page_contract {
    ...
    sim02 27/09/2016 Taranto ha il cod. impianto composto dalle ultim 3e cifre del codice istat
    sim02            + un progressivo

    nic03 05/02/2016 Gestito coimtgen.lun_num_cod_impianto_est per regione MARCHE
...
set page_title  "Inserimento impianto - Bonifica Impianti"
set context_bar [iter_context_bar -nome_funz $nome_funz_caller]
set mex_error ""

# preparo link per ritorna al filtro:
set link_list [export_url_vars caller nome_funz nome_funz_caller cod_comune search_word cod_impianto_est_new f_resp_cogn f_resp_nome f_comune f_cod_via f_desc_via f_desc_topo flag_tracciato]


# sproteggo la chiave solo in inserimento e gli attributi in inserimento e mod.
set form_name    "coimaimpins"
...
set form_valid {
	...
	if {$coimtgen(ente) eq "CPESARO" || $coimtgen(ente) eq "PPU" || $coimtgen(ente) eq "CFANO" || $coimtgen(ente) eq "CANCONA" || $coimtgen(ente) eq "PAN" || $coimtgen(ente) eq "CJESI" || $coimtgen(ente) eq "CSENIGALLIA"} {#sim01: aggiunta if ed il suo contenuto
	    ...
	    set progressivo_est [db_string sel_prog "select nextval('coimaimp_est_s')"]
	    # devo fare l'lpad con una seconda query altrimenti mi andava in errore
	    #nic03 set progressivo_est [db_string sel_prog "select lpad(:progressivo_est, 6, '0')"]
	    set progressivo_est [db_string sel_prog "select lpad(:progressivo_est,:lun_num_cod_imp_est,'0')"];#nic03
		...
		#sim01: la sel_dati_codifica andava in errore su lpad di progressivo. Quindi faccio lpad dopo la query
    ...
    set return_url   "coimaimp-gest?funzione=V&[export_url_vars flag_tracciato nome_funz_caller search_word cod_impianto_est_new]&nome_funz=impianti&flag_no_link=T&cod_impianto=$cod_impianto_new"   
