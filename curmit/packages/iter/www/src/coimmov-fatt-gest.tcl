...
set caus_pag [db_string query "select descrizione, id_caus from coimcaus where codice ='VC'"]
...
if {[form is_request $form_name]} {
	...
	if {[db_0or1row sel_aimp ""] == 0} {
        ...
        element set_properties $form_name id_caus          -value $caus_pag
...
if {[form is_valid $form_name]} {
    ...
    set id_caus [db_string query "select id_caus from coimcaus where codice ='VC'"]
    ...
    set link_gest      [export_url_vars cod_movi last_cod_movi nome_funz nome_funz_caller extra_par caller flag_filter]
    switch $funzione {
        M {set return_url   "coimmov-fatt-gest?funzione=V&$link_gest"}
        D {set return_url   "coimmov-fatt-list?$link_list"}
        I {set return_url   "coimmov-fatt-gest?funzione=V&$link_gest"}
        V {set return_url   "coimmov-fatt-list?$link_list"}
    }
