ad_page_contract {
    ...
    USER  DATA       MODIFICHE
    ===== ========== =======================================================================
    sim01 11/05/2020 Salvando i file nella permanenti è necessario fare il check del nome file
    sim01            non sulla spool ma sulla permanenti. Per fare ciò richiamo la iter_temp_file_name
    sim01            col -permanenti.
...
#sim01 set nome_file_tot    [iter_temp_file_name $nome_file_tot]
set nome_file_tot    [iter_temp_file_name -permanenti $nome_file_tot];#sim01
...
if {[string equal $cod_manutentore ""]} {
    ...
    set    utente_chk     [string range $id_utente 0 end-2]
    append utente_chk "__"
    set where_utente  "where (a.utente like :utente_chk or a.utente_ins like :utente_chk)"
}
...
# Per fare dei test, vanno commentate tutte le db_dml
...
with_catch error_msg {
	...
	if {$ctr_rec > 0} {
		...
		||  $anno_rif != $anno_controllo} {
		    ...
		    set nome_file       "Stampa distinta di consegna dichiarazioni di $nome_mese"
		    #sim01 set nome_file       [iter_temp_file_name $nome_file]
		    set nome_file       [iter_temp_file_name -permanenti $nome_file];#sim01
	...
	if {$ctr_rec_prescr > 0} {
		...
		||  $anno_rif != $anno_controllo} {
		    ...
		    #sim01 set nome_file2      [iter_temp_file_name $nome_file2]
		    set nome_file2      [iter_temp_file_name -permanenti $nome_file2];#sim01
