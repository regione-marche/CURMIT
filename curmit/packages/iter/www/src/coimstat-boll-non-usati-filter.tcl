ad_page_contract {
    ...
    serve se lista e' uno zoom che permetti aggiungi.
    @cvs-id          coimstat-boll-non-usati-filter.tcl

    USER   DATA       MODIFICHE
    ====== ========== =======================================================================
    sim01  19/05/2015 Corretto controllo sul filtro dei manutentori 
...
set titolo       "Parametri per stampa riepilogo bollini non utilizzati"
set button_label "Seleziona"
set page_title   "Parametri per stampa riepilogo bollini non utilizzati"
...
if {[form is_valid $form_name]} {
	...
	element::set_error $form_name f_manu_cogn "Selezionare un manutentore"
	incr error_num
    }
	...
	if {[string equal $chk_inp_cognome ""]} {
	    ...
	    set eq_cognome "= trim(upper(:chk_inp_cognome))"
	}
	if {[string equal $chk_inp_nome ""]} {
	    set eq_nome    "is null"
	} else {
	    set eq_nome    "= trim(upper(:chk_inp_nome))"
	}

	db_foreach sel_manu "select cod_manutentore as cod_manu_db
                               from coimmanu
                              where trim(upper(cognome))   $eq_cognome
                                and trim(upper(nome))      $eq_nome
                                and coalesce(flag_attivo,'') <> 'N' --sim01
    ...
    set return_url "coimstat-boll-non-usati-layout?$link"
