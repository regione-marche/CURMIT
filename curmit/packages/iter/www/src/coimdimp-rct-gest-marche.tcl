...
set controllo_tariffa {#sim08
    ...
    if {$coimtgen(ente) eq "PUD" || $coimtgen(ente) eq "PGO" || $coimtgen(ente) eq "PPN"} {#sim50
...
#sim53    }
	...
	set tariffa 15
	set esente "f"
    } elseif { ($coimtgen(ente) eq "PTA" || $coimtgen(ente) eq "PAN" ||  $coimtgen(ente) eq "CANCONA" ||  $coimtgen(ente) eq "CFANO" ||  $coimtgen(ente) eq "CJESI" ||  $coimtgen(ente) eq "CSENIGALLIA")  && $rcee_su_secondo_gen eq "t"} {;#sim43 #san13 #san14
	set tariffa 0
        set esente "t"
...
element create $form_name cognome_manu \
    ...
    -widget   text \
    -datatype text \
    -html    "size 15 maxlength 100 $readonly_fld {} class form_element $onchange_manu" \
    -optional
...
if {$coimtgen(regione) eq "MARCHE"} {#gac06 aggiunta if e else e loro contenuto
    set cod_manutentore [db_string q "select cod_manutentore from coimaimp where cod_impianto = :cod_impianto" -default ""]
	...
	-html    "$disabled_fld {} class form_element" \
	-optional \
	-options [iter_selbox_from_table_wherec coimstru_manu cod_strumento "marca_strum||' - '||modello_strum||' - '||matr_strum" "tipo_strum" "where cod_manutentore = '$cod_manutentore' and tipo_strum = '0'"]
	...
	-html    "$disabled_fld {} class form_element" \
	-optional \
	-options [iter_selbox_from_table_wherec coimstru_manu cod_strumento "marca_strum||' - '||modello_strum||' - '||matr_strum" "tipo_strum" "where cod_manutentore = '$cod_manutentore' and tipo_strum = '1'"]
...
element create $form_name warning_portata \
    ...
    -widget   inform \
    -datatype text \
    -html    "$disabled_fld_cod_tprc {} class form_element" \
    -optional 
...
if {[form is_request $form_name]} {
    ...
    element set_properties $form_name msg_rcee         -value "ATTENZIONE: poichè hai indicato che questo REE viene compilato in occasione della scadenza periodica prevista dall'Allegato 3 alla L.R. 19/2015, confermando l'inserimento applicherai anche il relativo segno identificativo."
	...
	if {$tipo_combustibile eq "G"} {
	    element set_properties $form_name rct_idonea_tenuta -value "C"
	    element set_properties $form_name rct_idonea_tenuta -after_html "<br>*Utilizzare la norma UNI 11137"
	}
	...
	while {$conta_prfumi < ($num_prove_fumi -1)} {#rom01 while e suo contenuto
	    ...
	    element set_properties $form_name rct_modulo_termico.$conta_prfumi -value 1;#gac03
	}
...
#nic03	element set_properties $form_name potenza          -value $potenza_old_edit
	...
	db_foreach a "select iter_edit_num(tiraggio, 2) as tiraggio_fumi
		...
		element set_properties $form_name warning_portata  -value "<font color=red><b>Scostamento della potenza max. al focolare superiore al 10%. Regolare il generatore prima di effettuare la misura del rendimento di combustione.</b></font>"
...
if {[form is_valid $form_name]} {
	...
	if {$changed_field eq "flag_status" || $changed_field eq "cod_tprc"} {;#sim19 if e suo contenuto
	...
	if {$cod_tprc eq "CADALLE" && $changed_field eq "cod_tprc"} {
	    element set_properties $form_name msg_rcee -value "ATTENZIONE: poichè hai indicato che questo REE viene compilato in occasione della scadenza periodica prevista dall'Allegato 3 alla L.R. 19/2015, confermando l'inserimento applicherai anche il relativo segno identificativo." 
	} else {
	    element set_properties $form_name msg_rcee -value ""
	}
	...
	if {$cont_rend == "S" && $conbustibile_solido eq "N" && $co <1000} {
	    #san06: aggiunta || "0"
	...
	if {$coimtgen(regione) eq "MARCHE"} {#gac01 aggiunta if e suo contenuto
	    if {$cod_strumento_01 eq ""} {#gac01
		element::set_error $form_name cod_strumento_01 "Inserire"
		incr error_num
	    };#gac01
	};#gac01

	if {$rend_magg_o_ugua_rend_min eq ""} {
            element::set_error $form_name rend_magg_o_ugua_rend_min "Inserire"
            incr error_num
        }

	if {$co > "1000" && $co_fumi_secchi eq "S"} {
	    element::set_error $form_name co_fumi_secchi "Impossibile valorizzare a Sì.<br>Il valore \"CO calc. (%)(ppm)\" è maggiore di 1000"
	    incr error_num
	}
	...
	if {$rend_combust < $rct_rend_min_legge && $rend_magg_o_ugua_rend_min eq "S"} {
	    element::set_error $form_name rend_magg_o_ugua_rend_min "Impossibile valorizzare a Sì.<br>Il valore di \"Rend.to di combustione(%)\" è minore di \"Rend.to combustione minimo di legge(%)\""
	    incr error_num
            }
	...
	while {$conta_prfumi < ($num_prove_fumi -1)} {#rom01 while e suo contenuto
	    ...
	    # fumi. per ora il bacharach non lo rendo obligatorio essendo un 
	    # valore riferito agli impianti a combustibile liquido.
	    if {$cont_rend == "S" && $conbustibile_solido eq "N" && $co_prfumi($conta_prfumi)<1000} {
		...
		if {[string equal $co_prfumi($conta_prfumi) ""]} {
 		...
 		if {$co_prfumi($conta_prfumi) > "1000" && $co_fumi_secchi eq "S"} {
		    element::set_error $form_name co_fumi_secchi "\"CO calc. (%)(ppm)\" è maggiore di 1000"
		    incr error_num
		}
		if {$rend_combust_prfumi($conta_prfumi) > $rct_rend_min_legge_prfumi($conta_prfumi) && $rend_magg_o_ugua_rend_min eq "S"} {
		    element::set_error $form_name rend_magg_o_ugua_rend_min "\"Rend.to di combustione(%)\" è minore di \"Rend.to combustione minimo di legge(%)\""
		    incr error_num
		}
	...
	if {![string equal $data_prox_manut ""]} {#san04: aggiunta if ed il suo contenuto
	    ...
	    if {$coimtgen(ente) eq "CANCONA" || $coimtgen(ente) eq "PAN" || $coimtgen(ente) eq "PFI"} {;#sim42
