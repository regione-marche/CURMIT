...
<queryset>
   ...
   select coimbpos.cod_bpos
	...
	, coimbpos.data_emissione
     ...
     from coimbpos
     join coiminco on coiminco.cod_inco      = coimbpos.cod_inco
...
left join coimcitt on coimcitt.cod_cittadino = coimaimp.cod_responsabile
 ...
 order by coimbpos.data_emissione desc
    ...
    <fullquery name="sel_bpos_sum">
       <querytext>
   select count(*)                                      as num_bpos
        , iter_edit_num(sum(coimbpos.importo_emesso),2) as tot_importo_emesso
        , iter_edit_num(sum(coimbpos.importo_pagato),2) as tot_importo_pagato
     from coimbpos 
     join coiminco on coiminco.cod_inco      = coimbpos.cod_inco
     join coimaimp on coimaimp.cod_impianto  = coiminco.cod_impianto
left join coimcitt on coimcitt.cod_cittadino = coimaimp.cod_responsabile
