ad_page_contract {
    Dettaglio tabella ucit_allfsc (allegati f senza codice - solo per UCIT - provincia di Udine)
    VISUALIZZAZIONE ALLEGATO F PER BONIFICHE

    @author          Nicola Mortoni - clonato da ucit-allgsc-gest.tcl
    @creation-date   22.03.2013

    @cvs-id          ucit-allfsc-gest.tcl
} {
    {id                   ""}
    {funzione            "V"}
    {caller          "index"}
...
set return_url "coim-r-f-dimp-admin-list?$link_list"
...
if {[form is_request $form_name]} {
    ...
    # leggo riga
    if {[db_0or1row sel_ucit_allfsc {}] == 0} {
...
if {[form is_valid $form_name]} {
    ...
    if {$funzione == "M"} {
	    ...
	    incr error_num
            ...
            if {$data_controllo == 0} {
                element::set_error $form_name data_controllo_pretty "Data controllo deve essere una data"
                incr error_num
            } else {
