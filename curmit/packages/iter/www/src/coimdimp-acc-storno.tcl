ad_page_contract {
    accettazione storno dichiarazioni (tabella "coimdimp")
    @author          dob
...
db_1row query "select a.cod_dimp
		  ...
		  , iter_edit_data(a.data_arrivo_ente) as data_arrivo_ente
                  ...
                  , coalesce(a.motivo_storno,'') as motivo_storno
...
if {[string equal $stato_dich ""]} {
    iter_return_complaint "
    Funzione annullata in quanto non e' stata inserita nessuna richiesta di storno."  
    ad_script_abort
...
if {[db_get_database] eq "iterrl-dev"} {
    set base_url "http://www.sviluppo.curit.it"
} else {
    set base_url "http://areaoperativa.curit.it"
}
...
if {[db_0or1row query "select 1 from coimdimp_stn where cod_dimp = :cod_dimp"] == 0} {
    set sostituto "Non e' presente nessuna dichiarazione  sostitutiva"
} else {
    set sostituto "La dichiarazione sostitutiva visibile a sistema prendera' il posto di quella stornata."
}

ns_sendmail "de.vincenzis@cestec.it" $uff_info "Richiesta storno dichiarazione" "Si richiede lo storno della dichiarazione n. $cod_dimp del $data_controllo_pretty relativa all'impianto $codice_impianto sito in $indirizzo nel comune di $comune .  Responsabile impianto $cognome_resp $nome_resp. La richiesta di storno e' stata inserita dal manutentore $cognome_manu $nome_manu con la seguente motivazione : $motivo_storno. $sostituto. Per effettuare lo storno eseguire il seguente programma $base_url/iter-portal/admin/storno?tran_id=0&cod_dimp=$cod_dimp&iter_dbn=$iter_dbn"

#B80 inserimento log per rich storno - start
#ns_sendmail "andrea_madonini@yahoo.it" $uff_info "CHECK-ACC-STORNO 1: Richiesta storno dichiarazione $cod_dimp" "Si richiede lo storno della dichiarazione n. $cod_dimp del $data_controllo_pretty relativa all'impianto $codice_impianto sito in $indirizzo nel comune di $comune .  Responsabile impianto $cognome_resp $nome_resp. La richiesta di storno e' stata inserita dal manutentore $cognome_manu $nome_manu con la seguente motivazione : $motivo_storno. $sostituto. Per effettuare lo storno eseguire il seguente programma $base_url/iter-portal/admin/storno?tran_id=0&cod_dimp=$cod_dimp&iter_dbn=$iter_dbn"
#set motivo_storno [subst [regsub -all {\n} $motivo_storno " "]]
regsub -all "\r\n" $motivo_storno " " motivo_storno
regsub -all "\n" $motivo_storno " " motivo_storno
#ns_sendmail "andrea_madonini@yahoo.it" $uff_info "CHECK-ACC-STORNO 2: Richiesta storno dichiarazione $cod_dimp" "Si richiede lo storno della dichiarazione n. $cod_dimp del $data_controllo_pretty relativa all'impianto $codice_impianto sito in $indirizzo nel comune di $comune .  Responsabile impianto $cognome_resp $nome_resp. La richiesta di storno e' stata inserita dal manutentore $cognome_manu $nome_manu con la seguente motivazione : $motivo_storno. $sostituto. Per effettuare lo storno eseguire il seguente programma $base_url/iter-portal/admin/storno?tran_id=0&cod_dimp=$cod_dimp&iter_dbn=$iter_dbn"
#B80 inserimento - end
...
#ns_log notice "prova dob1 destmail presa da coimuten : $destmail "
...
#ns_log notice "prova dob2 destmail presa da coimmanu se terzo resp. : $destmail "
...
#ns_log notice "prova dob3 destmail presa da coimcitt se amministratore : $destmail "
...
#ns_log notice "prova dob4 destmail presa da coimmanu : $destmail "
#ns_log notice "prova dob cod_manutentore $cod_manutentore"

if {[string equal $destmail ""] || [string equal $destmail "."]} {
    set url "$base_url/iter-portal/get-mail-manut?cod_manutentore=$cod_manutentore"
...
#ns_log notice "prova dob5 destmail manutentore $cod_manutentore presa da curit  : $destmail "

if {![string equal $destmail ""]} {
    ns_sendmail $destmail $uff_info "Accettazione richiesta storno dichiarazione" "Si comunica l'accettazione della richiesta di storno della dichiarazione n. $cod_dimp del $data_controllo_pretty relativa all'impianto $codice_impianto sito in $indirizzo nel comune di $comune .  Responsabile impianto $cognome_resp $nome_resp. La richiesta di storno e' stata inoltrata a CESTEC che provvedera' allo storno. $sostituto"
} else {
    ns_sendmail $uff_info $uff_info "Mancata spedizione mail di accettazione richiesta storno dichiarazione" "Si comunica la mancata comunicazione al responsabile dell'avvenuta l'accettazione della richiesta di storno della dichiarazione n. $cod_dimp del $data_controllo_pretty relativa all'impianto $codice_impianto sito in $indirizzo nel comune di $comune .  Responsabile impianto $cognome_resp $nome_resp. La richiesta di storno e' stata inoltrata a CESTEC che provvedera' allo storno. $sostituto"
}

db_dml query "update coimdimp set stato_dich = 'A' where cod_dimp = :cod_dimp"

set return_url $pack_dir/$directory/coimdimp-list?$link
ad_returnredirect -message "Richiesta di storno inoltrata a CESTEC" $return_url
ad_script_abort
