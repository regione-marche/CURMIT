...
switch $nome_funz_caller {
    insmodh {
	set page_title      "Inserimento mod. H - Bonifica impianti"
    }
    insmodhbis {
	set page_title      "Inserimento mod. H bis - Bonifica impianti"
    }
    insmodg {
	set page_title      "Inserimento mod. G - Bonifica impianti"
    }
    insmodf {
	set page_title      "Inserimento mod. F - Bonifica impianti"
    }
    insmodh-manu {
	set page_title      "Inserimento allegato H (per manutentori) - Bonifica impianti"
    }
    insmodi-manu {
	set page_title      "Inserimento allegato I (per manutentori) - Bonifica impianti"
    }
    insmodl-manu {
	set page_title      "Inserimento allegato L (per amministratori) - Bonifica impianti"
    }
    default {
	set page_title      "Inserimento modelli - Bonifica impianti"
    }
}
...
if {$flag_tracciato == "HMANU"
    || $flag_tracciato == "IMANU"
    || $flag_tracciato == "LMANU"} {
    set link_aggiungi   "<a href=\"coim_as_resp-gest?funzione=I&$link_isrt&nome_funz=[iter_get_nomefunz coim_as_resp-gest]\">Aggiungi nuova scheda non collegata a impianto"
} else {
	...
	set link_aggiungi "<a href=\"coimaimp-isrt-manu?funzione=I&$link_isrt&nome_funz=[iter_get_nomefunz coimaimp-isrt-manu]\">Aggiungi nuovo impianto collegato alla scheda selezionata"
    }
}
...
if {[string equal $cod_impianto_est_new ""]} {
    ...
    set where_imp  " and a.cod_impianto_est = :cod_impianto_est_new"
}
...
if {[form is_valid $form_name]} {
    ...
    if {$stato_aimp == "P"} { 
	set return_url   "coimdimp-gest?funzione=I&[export_url_vars flag_tracciato cod_as_resp nome_funz_caller search_word cod_impianto_est_new]&nome_funz=[iter_get_nomefunz coimdimp-gest]&flag_no_link=T&cod_impianto=$destinazione"
    } else {
	
	if {[ string equal $cod_as_resp ""]} {
	    set return_url   "coimdimp-ins-gest?[export_url_vars flag_tracciato destinazione compatta_list nome_funz nome_funz_caller f_comune cod_impianto_est_new f_resp_cogn f_resp_nome f_cod_via f_desc_via f_desc_topo search_word]"
	} else {
	    set return_url   "coimdimp-ins-gest?[export_url_vars flag_tracciato destinazione compatta_list nome_funz nome_funz_caller f_comune cod_impianto_est_new f_resp_cogn f_resp_nome f_cod_via f_desc_via f_desc_topo search_word cod_as_resp]"
	}
    }
