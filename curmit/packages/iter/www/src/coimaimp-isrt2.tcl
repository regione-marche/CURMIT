ad_page_contract {
    ...
    nic01 14/05/2014 Comune di Rimini: se è attivo il parametro flag_gest_coimmode, deve
...
set link_gest [export_url_vars cod_impianto last_cod_impianto nome_funz_caller extra_par caller]&nome_funz=impianti
...
# Controllo se l'utente è un comune
...
#sim01 devo visualizzare solo le potenze del caldo. Prima era [iter_selbox_from_table coimpote cod_potenza descr_potenza potenza_min]
...
#Setto la variabile nome_funz_new
...
set link_ins_terzi [iter_link_ins $form_name coimcitt-isrt [list cognome cognome_terzi nome nome_terzi nome_funz nome_funz_new dummy cod_citt_terzi flag_ins_terzi flag_ins_terzi] "Inserisci Sogg."]
...
if {[form is_request $form_name]} {
	...
	if {[string equal $coimtgen(flag_ente) "C"]} {
	    ...
	    element set_properties $form_name cod_comune -value $coimtgen(cod_comu)g
	...
	&&   [db_0or1row check_aimp ""] == 1
		...
		&&  [db_0or1row check_fascia_pote ""]== 0} {
		    ...
		    incr error_num
                ...
                if {![string equal $potenza ""]} {
		    if {($nome_funz_caller eq "insmodf" || $nome_funz_caller eq "insmodhbis") && $n_generatori eq 1} {
			if {$potenza <= 35} {
			    element::set_error $form_name potenza "Per l'inserimento di questo modello la potenza deve essere superiore a 35 kW"
			}	
		    }
...
#        }
	...
	&&  [string equal $nome_prop    ""]
        ...
        ########
	# se il soggetto occupante non c'è lo inserisco con l'indirizzo
        # uguale all'ubicazione dell'impianto. se il soggetto è presente 
        # con piu' omonimi, la prima volta visualizzo il messaggio, la 
        # seconda volta lascio inserire il soggetto.
        # se c'è un solo soggetto occupante con cognome e nome uguale (e
        ...
        # omonimo, invitando a verificare se il soggetto è corretto, in altro 
...
#		}
	...
	&&  [string equal $nome_manu    ""]
		...
		incr error_num
            ...
            if {$cod_mode eq ""} {#nic01
                element::set_error $form_name cod_mode "Inserire il modello";#nic01
       ...
       #} {
        ...
        #  element::set_error $form_name cod_impianto "Il record che stai tentando di inserire &egrave; gi&agrave; esistente nel Data Base."
        #  incr error_num
       #}
    }
    ...
    set return_url   "coimdimp-gest?funzione=I&[export_url_vars flag_tracciato nome_funz_caller search_word cod_impianto_est_new cod_impianto]&nome_funz=[iter_get_nomefunz coimdimp-gest]&flag_no_link=T"
