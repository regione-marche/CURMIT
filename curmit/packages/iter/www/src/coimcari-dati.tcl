...
#nome file finali
set comb_output "coimcomb.dat"
set topo_output "coimtopo.dat"
set viae_output "coimviae.dat"
set citt_output "coimcitt.dat"
set manu_output "coimmanu.dat"
set aimp_output "coimaimp.dat"
set cost_output "coimcost.dat"
set gend_output "coimgend.dat"
set cimp_output "coimcimp.dat"
set anom_output "coimanom.dat"
set dimp_output "coimdimp.dat"
set opve_output "coimopve.dat"
set prog_output "coimprog.dat"
...
set comb_output [open $dir/$output_dir/$comb_output w]
set topo_output [open $dir/$output_dir/$topo_output w]
set viae_output [open $dir/$output_dir/$viae_output w]
set citt_output [open $dir/$output_dir/$citt_output w]
set manu_output [open $dir/$output_dir/$manu_output w]
set aimp_output [open $dir/$output_dir/$aimp_output w]
set cost_output [open $dir/$output_dir/$cost_output w]
set gend_output [open $dir/$output_dir/$gend_output w]
set cimp_output [open $dir/$output_dir/$cimp_output w]
set anom_output [open $dir/$output_dir/$anom_output w]
set dimp_output [open $dir/$output_dir/$dimp_output w]
set opve_output [open $dir/$output_dir/$opve_output w]
set prog_output [open $dir/$output_dir/$prog_output w]
...
# Setto le variabili per il calcolo della data di scadenza delle autocertificazinoni
db_foreach sel_addmonth_dich "" {
    set month_modh $valid_mod_h
    set month_modh_b $valid_mod_h_b
}
...
foreach {indx_aimp note_impianto} [array get aimp_generale] {
    util_unlist $indx_aimp cod_impianto_est provenienza_dati descr_comb potenza potenza_utile cod_potenza data_installaz localita toponimo indirizzo numero esponente cod_comune provincia cap tipo_impianto flag_dpr412 flag_resp cognome_amm nome_amm natura_amm cfisc_amm indirizzo_amm numero_amm comune_amm sigla_amm cap_amm cognome_prop nome_prop natura_prop cfisc_prop indirizzo_prop numero_prop comune_prop sigla_prop cap_prop cognome_occu nome_occu natura_occu cfisc_occu indirizzo_occu civico_occu comune sigla_occu cap cognome_terzi nome_terzi natura_terzi cfisc_terzi indirizzo_terzi numero_terzi comune_terzi sigla_terzi cap_terzi cognome_intestatario nome_intestatario natura_intestatario cfisc_intestatario indirizzo_intestatario numero_intestatario comune_intestatario sigla_intestatario cap_intestatario cognome_manu nome_manu data_installaz data_attivaz data_rottamaz stato flag_dichiarato data_prima_dich data_ultim_dich consumo_annuo stato_conformita cod_cted tariffa scala piano interno cod_tpdu cod_qua cod_urb data_ins data_mod utente anno_costruzione marc_effic_energ volimetria_risc gb_x gb_y gen_prog n_generatori
    ...
    #leggo i valori dell'array assegnandoli alle variabili sotto elencate
#da modificare dopo
    set array_out_aimp($cod_impianto_est) [list $count_aimp $cod_combustibile $provenienza_dati $tipo_impianto $potenza $potenza_utile $cod_potenza $data_installaz $data_attivaz $data_rottamaz $stato $flag_dichiarato $data_prima_dich $data_ultim_dich $consumo_annuo $n_generatori $stato_conformita $cod_cted $tariffa $cod_responsabile $flag_resp $cod_intestatario $cod_proprietario $cod_occupante $cod_amministratore $cod_manutentore $localita $cod_via $toponimo $indirizzo $numero $esponente $scala $piano $interno $cod_comune $provincia $cap $cod_tpdu $cod_qua $cod_urb $data_ins $data_mod $utente $flag_dpr412 $anno_costruzione $marc_effic_energ $volimetria_risc $gb_x $gb_y $note_impianto]
...
# Preparazione per la scrittura del file degli impianti

foreach {cod_impianto_est valore_array} [array get array_out_aimp] { 

    util_unlist $valore_array count_aimp cod_combustibile provenienza_dati tipo_impianto potenza potenza_utile cod_potenza data_installaz data_attivaz data_rottamaz stato flag_dichiarato data_prima_dich data_ultim_dich consumo_annuo n_generatori stato_conformita cod_cted tariffa cod_responsabile flag_resp cod_intestatario cod_proprietario cod_occupante cod_amministratore cod_manutentore localita cod_via toponimo indirizzo numero esponente scala piano interno cod_comune provincia cap cod_tpdu cod_qua cod_urb data_ins data_mod utente flag_dpr412 anno_costruzione marc_effic_energ volimetria_risc gb_x gb_y note_impianto

    set data_scad_dich ""

    
    if {$data_ultim_dich eq ""} {
	if {[info exists datas_dich($cod_impianto_est)]} {
	    util_unlist $datas_dich($cod_impianto_est) data_dimp data_scad
	    
	    set data_ultim_dich $data_dimp
	    if {$data_scad_dich eq ""} {
		set data_scad_dich $data_scad
	    }
	}
    } else {
	if {$data_scad_dich eq ""} {
	    set data_dich $data_ultim_dich
	    if {$potenza < 35} {
		set months $month_modh
	    }
	    if {$potenza > 35} {
	    ...
	    db_1row add_date_month ""
	    set data_scad_dich $date
	}
    }


    puts $aimp_output "$count_aimp|$cod_impianto_est|$cod_combustibile|$provenienza_dati|$tipo_impianto|$potenza|$potenza_utile|$cod_potenza|$data_installaz|$data_attivaz|$data_rottamaz|$stato|$flag_dichiarato|$data_prima_dich|$data_ultim_dich|$consumo_annuo|$n_generatori|$stato_conformita|$cod_cted|$tariffa|$cod_responsabile|$flag_resp|$cod_intestatario|$cod_proprietario|$cod_occupante|$cod_amministratore|$cod_manutentore|$localita|$cod_via|$toponimo|$indirizzo|$numero|$esponente|$scala|$piano|$interno|$cod_comune|$provincia|$cap|$cod_tpdu|$cod_qua|$cod_urb|$data_ins|$data_mod|$utente|$flag_dpr412|$anno_costruzione|$marc_effic_energ|$volimetria_risc|$gb_x|$gb_y|$note_impianto|$data_scad_dich"
}

# Chiusura del file di output degli impianti
close $aimp_output
