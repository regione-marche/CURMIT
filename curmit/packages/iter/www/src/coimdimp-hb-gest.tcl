...
if {[form is_request $form_name]} {
	...
	if {[string equal $gen_prog ""]} {
		...
		if {$ctr_gend > 1} {
		    ...
		    set link_gend "flag_tracciato=HB&[export_url_vars cod_impianto last_cod_cimp nome_funz nome_funz_caller extra_par caller url_aimp url_list_aimp]"
...
if {[form is_valid $form_name]} {
	...
	if {![string equal $potenza ""]} {
        ...
        if {![string equal $data_arrivo_ente ""]} {
            ...
            if {$data_arrivo_ente == 0} {
                element::set_error $form_name data_arrivo_ente "Data arrivo all'ente deve essere una data"
                incr error_num
            }
        }
    ...
    switch $funzione {
        ...
        I {
	    if {$nome_funz_caller == "insmodhbis"} {
		set return_url   "$pack_dir/coimdimp-ins-filter?&nome_funz=insmodhbis&[export_url_vars flag_tracciato]&cod_impianto_old=$cod_impianto&cod_dimp_old=$cod_dimp"
	    } else {
