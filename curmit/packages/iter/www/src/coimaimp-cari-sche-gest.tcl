...
ns_log notice "prova dob 7"

    if {$error_num > 0} {
ns_log notice "prova dob 8"
        ad_return_template
        return
    }
  ...
  # Lancio la query di manipolazione dati contenute in dml_sql
    if {[info exists dml_sql]} {
        with_catch error_msg {
            db_transaction {
ns_log notice "prova dob 10"
                db_dml dml_coimbatc $dml_sql
            }
        } {
ns_log notice "prova dob 11"
    ...
    switch $funzione {
        I {set return_url   "coimaimp-cari-sche-gest?funzione=V&$link_gest"}
        V {set return_url   ""}
    }

ns_log notice "prova dob 12"
