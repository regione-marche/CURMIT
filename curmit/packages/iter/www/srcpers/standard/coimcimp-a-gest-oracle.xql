    ...
    <fullquery name="sel_gend">
       <querytext>
           select a.gen_prog_est       as gend_gen_prog_est
                ...
                , case
                     when a.tipo_foco = 'A' then 'Aperto'
                     when a.tipo_foco = 'C' then 'Chiuso'
                     else 'Non Noto'
                  end                  as gend_tipo_focolare
                , case 
                     when a.tiraggio = 'F' then 'Forzato'
                     when a.tiraggio ='N' then 'Naturale'
                     else 'Non Noto'
                  end                  as gend_tiraggio
                , a.modello            as gend_modello
                , a.matricola          as gend_matricola
                , case 
                     when a.tipo_bruciatore = 'A' then 'Atmosferico'
                     when a.tipo_bruciatore = 'P' then 'Soffiato'
                     when a.tipo_bruciatore = 'M' then 'Premiscelato'
                     else 'Non Noto'
                ...
                , case 
                      when a.locale = 'T' then 'Tecnico'
                      when a.locale = 'E' then 'Esterno'
                      when a.locale = 'I' then 'Interno'
                      else 'Non Noto'
                  end                  as gend_tipologia_locale
                , g.descr_fuge         as gend_fluido_termovettore
                , case 
                     when a.cod_emissione = '0' then 'Canna fumaria non verificabile'
                     when a.cod_emissione = 'C' then 'Canna fumaria collettiva al tetto'
                     when a.cod_emissione = 'I' then 'Canna fumaria singola al tetto'
                     when a.cod_emissione = 'P' then 'Scarico direttamente all''''esterno'
                     else 'Non Noto'
    ...
    <partialquery name="upd_aimp_pote">
       <querytext>
	      ...
	      set potenza          = :pot_focolare_nom_upd
	        ...
	        , cod_combustibile = :cod_combustibile_upd
                , data_mod         =  sysdate
                , utente           = :id_utente
            where cod_impianto     = :cod_impianto
       </querytext>
