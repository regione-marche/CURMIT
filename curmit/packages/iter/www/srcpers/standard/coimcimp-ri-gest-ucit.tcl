...
# double click protection
    ...
    if {$i > 1} {
        #double click
        ad_return_complaint 1 "Protezione da doppio click: l' aggiornamento � gi� stato eseguito.
...
if {[form is_valid $form_name]} {
    ...
    set potenza_effettiva_util  [element::get_value $form_name potenza_effettiva_nom]
...
db_1row sel_contr_cimp "select count(*) as cont_cimp from coimcimp a  where a.cod_inco = :cod_inco and a.cod_impianto = :cod_impianto"


   if {$cont_cimp > 1 } {
   	    element::set_error $form_name cod_inco  "Non si possono inserire pi� rapporti di controllo per la stesso appuntamento"
 		    incr error_num
	}

}
      if {[string equal $cod_inco ""]} {
    	    element::set_error $form_name cod_inco  "Codice appuntamento non pu� essere nullo"
		    incr error_num
	}
...
if {[string equal $frequenza_manut "T"] & [string equal $frequenza_manut_altro ""]} {
	...
	if {$conta_pericoloso > 0} {
	    append note_conf " - SI DIFFIDA IL RESPONSABILE IMPIANTO AD EFFETTUARE ENTRO 60 GIORNI DA OGGI, GLI INTERVENTI NECESSARI AD ELIMINARE LE SEGUENTI IRREGOLARITA', SEGUIRA' LETTERA PER LA SECONDA VISITA DI CONTROLLO (RILASCIATA DICHIARAZIONE - MOD. ROSA)"
	} else {
	    if {$conta_gen_esito == 0} {
		append note_conf ""
	    } else {
		append note_conf " - SI DIFFIDA IL RESPONSABILE IMPIANTO AD EFFETTUARE ENTRO 60 GIORNI DA OGGI, GLI INTERVENTI NECESSARI AD ELIMINARE LE SEGUENTI IRREGOLARITA' (RILASCIATA DICHIARAZIONE 60 GIORNI - MOD. VERDE)"
	    }
	}
...
#	}
	...
	if {$is_warning_p1 eq "f" && $error_num <= 0} {
		...
		set is_warning_p1 "t"
		element set_properties $form_name is_warning_p1  -value $is_warning_p1
		element::set_error $form_name costo "Sei sicuro che sia un controllo statistico ? Se si, effettua di nuovo l'aggiornamento"
		incr error_num
	    }
	}
    }
...
# double click protection del 100513
    ...
    if {$i > 1} {
        #double click
        iter_return_complaint  "Protezione da doppio click: l' aggiornamento � gi� stato eseguito.
                               Quando tornerai indietro dovrai rinfescare la pagina per tener
                               conto dell'aggiornamento effettuato."
        ad_script_abort
    }
    ...
    if {[info exists dml_sql]} {
		...
		if {[info exists dml_ins_anom]} {
			...
			if {![string equal $cod_anom($conta) ""]} {
				...
				# inserisco l'anomalia solo se 'flag_stp_esito' è 'T'
