...
set titolo           "Rapporto di verifica < 35 kW"
switch $funzione {
...
-html    "size 1 maxlength 1 $disabled_fld {} class form_element" \
-optional \
-options  {{{} {}} {Buono B} {Mediocre M} {Scarso S} {NV V}}
...
element create $form_name new1_foro_corretto \
-label   "pendenza corrette" \
-widget   select \
-datatype text \
-html    "$disabled_fld {} class form_element" \
...
element create $form_name new1_foro_accessibile \
-label   "pendenza corrette" \
-widget   select \
-datatype text \
-html    "$disabled_fld {} class form_element" \
...
element create $form_name ventilaz_lib_ostruz \
...
element create $form_name disp_reg_clim_funz \
...
element create $form_name strumento \
-label   "strumento" \
-widget   text \
-datatype text \
...
element create $form_name marca_strum \
-label   "marca strumento" \
-widget   text \
-datatype text \
-html    "size 10 maxlength 50 $readonly_fld {} class form_element" \
-optional

element create $form_name modello_strum \
...
element create $form_name matr_strum \
...
element create $form_name new1_canali_a_norma \
-label   "new1_canali_a_norma" \
-widget   select \
-datatype text \
-html    "size 1 maxlength 1 $disabled_fld {} class form_element" \
-optional \
-options  {{{} {}} {Buono S} {Mediocre M} {{Non idoneo} N}}

element create $form_name spazi_norma \
-label   "spazi_norma" \
-widget   select \
-datatype text \
-html    "size 1 maxlength 1 $disabled_fld {} class form_element" \
...
#   element set_properties $form_name cod_inco       -value $cod_inco
	...
	set h_potenza [iter_check_num $pot_focolare_nom 2]
	...
	# se esite il verificatore vado a reperire i dati dello strumento
        # e li imposto come default. 
	if {![string equal $inco_cod_opve ""]} {
	    set where_opve "where cod_opve = :inco_cod_opve"
	    if {[db_0or1row sel_opve_strum ""] == 0} {
		set strumento_opve     ""
		set marca_strum_opve   ""
		set modello_strum_opve ""
                set matr_strum_opve    ""
	    }
	} else {
	    set strumento_opve     ""
	...
	element set_properties $form_name gen_prog              -value $gen_prog
	element set_properties $form_name pot_utile_nom         -value $gend_pot_utile_nom
	element set_properties $form_name pot_focolare_nom      -value $gend_pot_focolare_nom
	...
	element set_properties $form_name dich_conformita       -value "S"
	element set_properties $form_name libretto_manutenz     -value "S"
	...
	element set_properties $form_name tipo_foco           -value $gend_tipo_focolare
	element set_properties $form_name mod_funz            -value $gend_mod_funz
	element set_properties $form_name dpr_660_96          -value $gend_dpr_660_96
	element set_properties $form_name tiraggio            -value $gend_tiraggio
	element set_properties $form_name data_installaz      -value $gend_data_installaz
	element set_properties $form_name locale              -value $gend_locale
	element set_properties $form_name cod_tpdu            -value $aimp_dest_uso
	element set_properties $form_name cod_tpim            -value $aimp_tipologia
	element set_properties $form_name cod_utgi            -value $gend_destinazione_uso
	...
	element set_properties $form_name new1_foro_corretto  -value $new1_foro_corretto
	...
	element set_properties $form_name data_verb           -value $data_verb
        ...
        element set_properties $form_name ventilaz_lib_ostruz -value $ventilaz_lib_ostruz
        ...
        element set_properties $form_name disp_reg_clim_funz  -value $disp_reg_clim_funz
	...
	element set_properties $form_name strumento           -value $strumento
	element set_properties $form_name marca_strum         -value $marca_strum
	element set_properties $form_name modello_strum       -value $modello_strum
	element set_properties $form_name matr_strum          -value $matr_strum
	...
	element set_properties $form_name new1_canali_a_norma -value $new1_canali_a_norma
	element set_properties $form_name spazi_norma         -value $spazi_norma
...
if {[form is_valid $form_name]} {
  # form valido dal punto di vista del templating system
    ...
    set new1_foro_corretto      [element::get_value $form_name new1_foro_corretto]
    ...
    set ventilaz_lib_ostruz     [element::get_value $form_name ventilaz_lib_ostruz]
    ...
    set disp_reg_clim_funz      [element::get_value $form_name disp_reg_clim_funz]
    ...
    set strumento               [element::get_value $form_name strumento]
    set marca_strum             [element::get_value $form_name marca_strum]
    set modello_strum           [element::get_value $form_name modello_strum]
    set matr_strum              [element::get_value $form_name matr_strum]
    ...
    set new1_canali_a_norma     [element::get_value $form_name new1_canali_a_norma]
    set spazi_norma             [element::get_value $form_name spazi_norma]
  ...
  # controlli standard su numeri e date, per Ins ed Upd
	...
	if {[string equal $cod_tpdu ""]} {
            element::set_error $form_name cod_tpdu "Inserire"
            incr error_num	
	}

	if {[string equal $cod_tpim ""]} {
            element::set_error $form_name cod_tpim "Inserire"
            incr error_num	
	}
    
	if {[string equal $cod_utgi ""]} {
            element::set_error $form_name cod_utgi "Inserire"
            incr error_num	
	}
	...
	if {[string equal $cod_emissione ""]} {
            element::set_error $form_name cod_emissione "Inserire"
            incr error_num	
	}
	...
	if {![string equal $temp_fumi_3a ""]} {
        ...
        if {![string equal $co_1a ""]} {
            ...
            if {$co_1a == "Error"} {
                ...
                if {[iter_set_double $co_1a] >=  [expr pow(10,4)]
                ||  [iter_set_double $co_1a] <= -[expr pow(10,4)]} {
                    element::set_error $form_name co_1a "Deve essere inferiore di 10.000"
                    incr error_num
                }
            }
        }
        if {![string equal $co_2a ""]} {
            ...
            if {$co_2a == "Error"} {
                ...
                if {[iter_set_double $co_2a] >=  [expr pow(10,4)]
                ||  [iter_set_double $co_2a] <= -[expr pow(10,4)]} {
                    element::set_error $form_name co_2a "Deve essere inferiore di 10.000"
                    incr error_num
                }
            }
        }
        if {![string equal $co_3a ""]} {
            ...
            if {$co_3a == "Error"} {
                ...
                if {[iter_set_double $co_3a] >=  [expr pow(10,4)]
                ||  [iter_set_double $co_3a] <= -[expr pow(10,4)]} {
                    element::set_error $form_name co_3a "Deve essere inferiore di 10.000"
                    incr error_num
                }
            }
        }
		...
		if {$data_installaz > $current_date} {
		    element::set_error $form_name data_installaz "Data installazione deve essere inferiore alla data odierna"
		    incr error_num
		}
	    }
        }
	...
	if {[string equal $stato_coiben ""]} {
	    element::set_error $form_name stato_coiben "Inserire"
	    incr error_num		
	}
...
#	ns_return 200 text/html $__refreshing_p; return
    ...
    # se i dati dello strumento non sono valorizzati o non sono stati valorizzati in precedenza perche' 
    # la verifica non deriva da un'incontro gia' assegnato ad un verificatore,
    # vado a reperire quelli del verificatore
    if {$funzione == "I"
    &&	[string equal $strumento ""]
    &&  [string equal $marca_strum ""]
    &&  [string equal $modello_strum ""]
    &&  [string equal $matr_strum ""]
    } {
	set where_opve "where cod_opve = :cod_opve"
	if {[db_0or1row sel_opve_strum ""] == 1} {
	    set strumento     $strumento_opve
	    set marca_strum   $marca_strum_opve
	    set modello_strum $modello_strum_opve
	    set matr_strum    $matr_strum_opve
	}
    }
	...
	# controllo se esiste un rapporto di verifica con data pi� recente, in questo caso non vado 
	...
	if {$data_controllo > $data_ultimo_cimp} {
		...
		set gend_pot_utile_nom [iter_check_num $gend_pot_utile_nom 2]
    ...
    switch $funzione {
	    ...
	    set dml_sql_gend [db_map upd_gen]
	    set dml_sql_aimp [db_map upd_imp]
