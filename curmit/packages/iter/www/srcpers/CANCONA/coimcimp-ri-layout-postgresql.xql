...
<!--
    USER  DATA       MODIFICHE
    ===== ========== =========================================================================
    rom02 19/04/2021 Aggiunto il case per la frequenza semestrale del campo frequenza_manut.
...
<queryset>
    ...
    <fullquery name="sel_cimp_si_vie">
       <querytext>
                select a.cod_impianto
                     ...
                     , coalesce(c.descr_topo,'')||' '||coalesce(c.descr_estesa,'')||' '||coalesce(b.numero,'')||' '||coalesce(b.esponente, '')||' S.'||coalesce(b.scala,'')||' P.'|| coalesce(b.piano, '') as ubicazione
		       ...
		       when 'V' then 'NV'
                     ...
                     , coalesce(iter_edit_num(f.campo_funzion_min, 2) , ' ') as gend_campo_funzion_min_edit
                     , coalesce(iter_edit_num(f.campo_funzion_max, 2) , ' ') as gend_campo_funzion_max_edit
                     , f.cod_emissione 
                     ...
                     , coalesce(iter_edit_num(a.temp_h2o_out_1a, 2), '&nbsp;') as temp_h2o_out_1a
                     , coalesce(iter_edit_num(a.temp_h2o_out_2a, 2),'&nbsp;') as temp_h2o_out_2a
                     , coalesce(iter_edit_num(a.temp_h2o_out_3a, 2),'&nbsp;') as temp_h2o_out_3a
                     , coalesce(iter_edit_num(a.t_aria_comb_1a, 2),'&nbsp;') as t_aria_comb_1a
                     , coalesce(iter_edit_num(a.t_aria_comb_2a, 2),'&nbsp;') as t_aria_comb_2a
                     , coalesce(iter_edit_num(a.t_aria_comb_3a, 2),'&nbsp;') as t_aria_comb_3a
                     , coalesce(iter_edit_num(a.temp_fumi_1a, 2),'&nbsp;') as temp_fumi_1a
                     , coalesce(iter_edit_num(a.temp_fumi_2a, 2),'&nbsp;') as temp_fumi_2a
                     , coalesce(iter_edit_num(a.temp_fumi_3a, 2),'&nbsp;') as temp_fumi_3a
                     , coalesce(iter_edit_num(a.co_1a, 2),'&nbsp;') as co_1a
                     , coalesce(iter_edit_num(a.co_2a, 2),'&nbsp;') as co_2a
                     , coalesce(iter_edit_num(a.co_3a, 2),'&nbsp;') as co_3a
                     , coalesce(iter_edit_num(a.co2_1a, 2),'&nbsp;') as co2_1a
                     , coalesce(iter_edit_num(a.co2_2a, 2),'&nbsp;') as co2_2a
                     , coalesce(iter_edit_num(a.co2_3a, 2),'&nbsp;') as co2_3a
                     , coalesce(iter_edit_num(a.o2_1a, 2),'&nbsp;') as o2_1a
                     , coalesce(iter_edit_num(a.o2_2a, 2),'&nbsp;') as o2_2a
                     , coalesce(iter_edit_num(a.o2_3a, 2),'&nbsp;') as o2_3a
                     , coalesce(iter_edit_num(a.eccesso_aria_perc, 2),'&nbsp;') as eccesso_aria_perc
                     , coalesce(iter_edit_num(a.perdita_ai_fumi, 2),'&nbsp;') as perdita_ai_fumi
                     , coalesce(iter_edit_num(b.volimetria_risc, 2),'&nbsp;') as aimp_volumetria_risc
                     , coalesce(iter_edit_num(b.consumo_annuo, 2),'&nbsp;') as aimp_consumo_annuo
                     , case f.dpr_660_96
                     ...
                     , case a.scarico_dir_esterno
                      ...
                      , coalesce(iter_edit_num(a.et, 2),'&nbsp;') as et
                      , fl_firma_tecnico         
	              , fl_firma_resp     
		      , fl_rifiuto_firma 
                      , coalesce(iter_edit_num(a.imp_boll_ver, 2),'&nbsp;') as imp_boll_ver  
                      , numfatt
			  ...
			  when 'S' then 'Semestrale' --rom02
                      ...
                      , case check_valvole
		      when 'S' then 'S&igrave;'
		      else '' end as check_valvole
                      , case check_isolamento
		      when 'S' then 'S&igrave;'
		      else '' end as check_isolamento
                      , case check_trattamento
		      when 'S' then 'S&igrave;'
		      else '' end as check_trattamento
                      , case check_regolazione
		      when 'S' then 'S&igrave;'
		      else '' end as check_regolazione
	              ...
	              , a.ora_inizio
	              , coalesce(iter_edit_num(a.potenza_nom_tot_foc, 2),'&nbsp;') as potenza_nom_tot_foc
	              , coalesce(iter_edit_num(a.potenza_nom_tot_util, 2),'&nbsp;') as potenza_nom_tot_util
		      ...
		      , a.rapp_cont_inviato
	              , a.rapp_cont_bollino
		     , b.toponimo || b.indirizzo as indirizzo
		     , coalesce(m.denominazione,'') as descr_comu
		     , coalesce(b.localita,'') as localita
		     , case int_rend_term
		     when 'T' then 'Non sono stati individuati initerventi economicamente convenienti'
		     when 'C' then 'Si allega relazione di dettaglio'
		     when 'S' then 'Si allega relazione di dettaglio'
		     when 'N' then 'NON ESEGUITO'
		     else ' ' end as int_rend_term
		     , int_rend_term_note
		     , b.n_generatori
		     , case f.tipo_foco
		     when 'B' then 'Singolo tipo B'
		     when 'C' then 'Singolo tipo C'
		     when 'M' then 'Moduliare'
		     when 'T' then 'Tubo o nastro radiante'
		     when 'A' then 'Ad aria calda'
		     else '' end as gend_tipo_foco
		  from coimcimp a
    ...
    <fullquery name="sel_cimp_no_vie">
       <querytext>
                select coalesce(b.toponimo,'')||' '||coalesce(b.indirizzo,'')||' '||coalesce(b.numero,'')||' '||coalesce(b.esponente,'')||' S.'||coalesce(b.scala,'')||' P.'|| coalesce(b.piano, '') as ubicazione
                     , a.cod_impianto
                     , a.cod_documento
                     , a.verb_n
                     , iter_edit_data(a.data_controllo) as data_controllo
                     ...
                     , iter_edit_data(b.data_installaz) as data_installaz
                     , d.cognome as cogn_responsabile
                     , coalesce(d.nome, '&nbsp;') as nome_responsabile
                     , coalesce(d.indirizzo,' ')||'&nbsp;'||coalesce(d.numero,' ') as indi_resp
                     , d.comune as comu_resp
                     , coalesce(d.telefono,' ')||'&nbsp;'||coalesce(d.cellulare,' ') as telef_resp
                     , n.cognome as cogn_amministratore
		     ...
		     when 'A' them 'Ad aria calda'
                     ...
                     , coalesce(iter_edit_num(a.volumetria, 2),'&nbsp;') as volumetria
                     , coalesce(iter_edit_num(a.comsumi_ultima_stag, 2),'&nbsp;') as comsumi_ultima_stag
                     , coalesce(iter_edit_num(a.temp_h2o_out_1a, 2),'&nbsp;') as temp_h2o_out_1a
                     , coalesce(iter_edit_num(a.temp_h2o_out_2a, 2),'&nbsp;') as temp_h2o_out_2a
                     , coalesce(iter_edit_num(a.temp_h2o_out_3a, 2),'&nbsp;') as temp_h2o_out_3a
                     , coalesce(iter_edit_num(a.t_aria_comb_1a, 2),'&nbsp;') as t_aria_comb_1a
    ...
    <fullquery name="sel_docu_contenuto">
       <querytext>
         select --rom01 contenuto as docu_contenuto_check
	        path_file as docu_contenuto_check --rom01 
            from coimdocu
           where cod_documento = :cod_documento
       </querytext>
    </fullquery>
    ...
    <partialquery name="upd_docu_2">
       <querytext>
                   update coimdocu
		   --rom01                      set contenuto     = lo_unlink(coimdocu.contenuto)
		      set path_file = null --rom01
                    where cod_documento = :cod_documento
       </querytext>
    </partialquery> 
    ...
    <partialquery name="upd_docu_3">
       <querytext>
             ...
             --rom01 , contenuto      = lo_import(:contenuto_tmpfile)
                     , path_file      = :file_pdf --rom01
                 where cod_documento  = :cod_documento
       </querytext>
    </partialquery> 
