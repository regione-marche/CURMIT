ad_page_contract {
    ...
    @cvs-id                 coimcimp-ri-gest.tcl

    USER  DATA       MODIFICHE
    ===== ========== ==========================================================================
    gac01 22/11/2019 Per poter aggiungere o modificare una dichiarazione bisogna aver inserito
    gac01            tutti i dati obbligatori

    rom02 31/07/2019 La regione Marche ha chiesto di uniformare i Rapporti di Ispezione con il
    rom02            loro modello regionale, quindi ho messo il programmma tra i personalizzati,
    rom02            ho aggiunto dei campi, ho modificato l'impaginazione e la stampa.

    sim02 10/06/2019 Al momento dell'inserimento del rapporto di ispezione vado a sbloccare
    sim02            l'inserimento degli rcee.

    rom01 25/01/2019 Sandro ha detto che, per ogni appuntamento, può essere inserito un Rapporto 
    rom01            d'Ispezione per ogni generatore presente sull'impianto, prima faceva 
    rom01            inserire un solo Rapporto per ogni appuntamento.

    sim01 20/07/2015 L'ing. Pepe di Agenzia Fiorentina per l'Energia ha chiesto di mettere un  
    sim01            controllo che se esiste almeno una anomalia l'esito non può essere positivo.
    sim01            Pepe chiede anche ditogliere il controllo 'Sei sicuro che sia un controllo 
    sim01            statistico ? Se si, effettua di nuovo l'aggiornamento'

    nic01 07/07/2015 L'ing. Pepe di Agenzia Fiorentina per l'Energia ha chiesto di fare in modo
    nic01            di non valorizzare automaticamente l'esito a negativo nel caso in cui
    nic01            venga indicato che la manutenzione dell'anno in corso non e' stata
    nic01            effettuata (punto 7, campo manutenzione_8a) perche' per loro hanno tempo
    nic01            tutto l'anno solare per effettuarla (anche dopo la verifica).
...
#rom02
element create $form_name rapp_cont_inviato \
    -label   "rapporto di conrollo efficienza energetica inviato" \
    -widget   select \
    -datatype text \
...
#rom02
element create $form_name rapp_cont_bollino \
    -label   "bollino presente" \
    -widget   select \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
    -optional \
...
element create $form_name indirizzo \
    ...
    -optional
#rom02
element create $form_name int_rend_term \
    -label   "Interventi atti  a migliorare il rendimento energetico" \
    -widget   select \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
    -optional \
    -options {{{Non sono stati individuati initerventi economicamente convenienti} T} {{Si allega relazione di dettaglio} C} {{Si rimanda a relazione di dettaglio successiva} S} {{Valutazione non eseguita} N} }
#rom02
element create $form_name int_rend_term_note \
    -label   "note Interventi atti  a migliorare il rendimento energetico" \
    -widget   textarea \
    -datatype text \
...
element create $form_name dimensionamento_gen \
    ...
    -html    "$disabled_fld {} class form_element" \
    -optional \
    -options {{{Non Controllabile} T} {{Dimensionamento corretto} C} {{Dimensionamento non corretto} N} {{Si rimanda a relazione di dettaglio successiva} S}}
...
element create $form_name tipo_foco \
    ...
    -html    "$disabled_fld {} class form_element" \
    -optional \
    -options {{{} {}} {{Singolo tipo B} B} {{Singolo tipo C} C} {Modulare M} {{Tubo o nastro radiante} T} {{Ad aria calda} A} }
...
element create $form_name is_controllo_ok  -widget hidden -datatype text -optional;#gac01
...
if {[form is_request $form_name]} {
    if {!$is_controllo_ok && $coimtgen(regione) eq "MARCHE"} {#gac01 aggiunta if e suo contenuto
    set caller "cimp"
    set link   [export_ns_set_vars "url"]
    set link   [export_url_vars link]

    ad_returnredirect [ad_conn package_url]src/coimaimp-warning?cod_impianto=$cod_impianto&redirect=coimcimp-ri-gest&$link&funzione=$funzione&caller=$caller
    ad_script_abort    
}

    element set_properties $form_name is_controllo_ok      -value $is_controllo_ok;#gac01
   ...
   element set_properties $form_name gen_prog       -value $gen_prog
	...
	if {[string equal $gen_prog ""]} {
		...
		if {$ctr_gend > 1} {
		    ...
		    set link_gend "flag_tracciato=RI&[export_url_vars cod_impianto last_cod_cimp nome_funz nome_funz_caller extra_par caller url_aimp url_list_aimp flag_cimp flag_inco extra_par_inco cod_inco]"
	...
	element set_properties $form_name flag_modifica        -value $flag_modifica
        ...
        element set_properties $form_name gen_prog             -value $gen_prog
	element set_properties $form_name rapp_cont_inviato    -value $rapp_cont_inviato;#rom02
	element set_properties $form_name rapp_cont_bollino    -value $rapp_cont_bollino;#rom02
	element set_properties $form_name int_rend_term        -value $int_rend_term;#rom02
	element set_properties $form_name int_rend_term_note   -value $int_rend_term;#rom02
        element set_properties $form_name cod_inco             -value $cod_inco
...
if {[form is_valid $form_name]} {
    ...
    set is_controllo_ok [element::get_value $form_name is_controllo_ok];#gac01
    set cod_cimp                [element::get_value $form_name cod_cimp]
    set gen_prog                [element::get_value $form_name gen_prog]
    set rapp_cont_inviato       [element::get_value $form_name rapp_cont_inviato];#rom02
    set rapp_cont_bollino       [element::get_value $form_name rapp_cont_bollino];#rom02
    set int_rend_term           [element::get_value $form_name int_rend_term];#rom02
    set int_rend_term_note      [element::get_value $form_name int_rend_term_note];#rom02
    element set_properties $form_name cod_impianto_est            -value $cod_impianto_est;#rom02
    #set cod_impianto_est        [element::get_value $form_name cod_impianto_est];#rom02
    element set_properties $form_name localita             -value $localita;#rom02
    element set_properties $form_name indirizzo            -value $indirizzo;#rom02
    element set_properties $form_name descr_comune         -value $descr_comune;#rom02
    element set_properties $form_name descr_flag_resp      -value $descr_flag_resp;#rom02
...
# double click protection del 100513
    ...
    if {[info exists dml_sql]} {
		...
		if {[info exists dml_upd_aimp_installaz]} {
		    db_dml dml_coimaimp_installaz $dml_upd_aimp_installaz
		}
