...
set prep_link_dimp {
    ...
    set link_dimp [export_url_vars cod_opma cod_impianto data_ins url_gage nome_funz_caller cod_dimp]&nome_funz=[iter_get_nomefunz coimdimp-gest-palm]&flag_no_link=T
}
...
-widget   textarea \
-datatype text \
-html    "cols 18 rows 2 $readonly_fld {} class form_element" \
-optional
...
if {[form is_valid $form_name]} {
  ...
  # controlli standard su numeri e date, per Ins ed Upd
	...
	if {![string is space $data_esecuzione]} {
	    ...
	    if {[db_0or1row sel_dimp_check ""] == 1} {
		element::set_error $form_name stato_ed "Il controllo non &egrave; eliminabile: &egrave; presente un modello H correlato"
		incr error_num
	    }
	}
    }
  ...
  # Lancio la query di manipolazione dati contenute in dml_sql
    ...
    switch $funzione {
        M {set return_url   "coimgage-gest-palm?funzione=V&$link_gest"}
        D {set return_url   "coimgage-list-palm?$link_list"}
        V {set return_url   "coimgage-list-palm?$link_list"}
    }
