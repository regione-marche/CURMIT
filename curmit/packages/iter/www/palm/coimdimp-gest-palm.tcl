...
set dett_tab [iter_tab_form_palm $cod_impianto]
...
||  $funzione == "M"
    ...
    set cerca_opma [iter_search $form_name [ad_conn package_url]/src/coimopma-list [list cod_manutentore cod_manutentore dummy cod_opmanu f_cognome nome_opma f_nome cognome_opma]]
} else {
    set cerca_opma ""
}
...
||  $funzione == "M"
    ...
    set cerca_resp [iter_search $form_name coimcitt-filter [list dummy cod_responsabile f_cognome cognome_resp f_nome nome_resp]]
} else {
    set cerca_resp ""
}
...
||  $funzione == "M"
    ...
    set cerca_occu [iter_search $form_name coimcitt-filter [list dummy cod_occupante f_cognome cognome_occu f_nome nome_occu]]
} else {
    set cerca_occu ""
}
...
-widget   text \
-datatype text \
-html    "size 5 maxlength 8 $readonly_fld {} class form_element" \
-optional
...
-widget   text \
-datatype text \
-html    "size 5 maxlength 6 $readonly_fld {} class form_element" \
-optional
...
-widget   text \
-datatype text \
-html    "size 6 maxlength 8 $readonly_fld {} class form_element" \
-optional
...
-widget   text \
-datatype text \
-html    "size 5 maxlength 14 $readonly_fld {} class form_element" \
-optional
...
-widget   textarea \
-datatype text \
-html    "cols 30 rows 3 $readonly_fld {} class form_element" \
-optional
...
-widget   text \
-datatype text \
-html    "size 15 maxlength 50 $readonly_fld {} class form_element" \
-optional
...
-html    "$disabled_fld {} class form_element" \
-optional \
-options {{{} {}} {{Bollino Prepagato} BO} {{Bollettino postale} BP} {{Contante a sportello ente} CN} {{Bonifico Bancario} BB} {{Carta di Credito} CC} {{POS} PS}}
...
while {$conta < 5} {
    ...
    -html     "$disabled_fld {} class form_element" \
    -optional \
    -options [iter_selbox_from_table coimtano cod_tano "cod_tano"]
...
element create $form_name cod_opmanu       -widget hidden -datatype text -optional
...
if {$funzione != "I"
    ...
    set esito "<tr>
             <td width=50% >Esito controllo: $esit</td>
              </tr>"   
} else {
    set esito ""
}
...
if {[form is_request $form_name]} {
	...
	# valorizzo il default del tipo pagamento e di pagato
	# personalizzazione per provincia di Mantova, menu' cooperativa
	set flag_coop "F"
	if {$coimtgen(flag_ente)     == "P"
	&&  $coimtgen(sigla_prov)    == "MN"
	} {
	...
	while {$conta < 5} {
       ...
       element set_properties $form_name cod_opmanu       -value $cod_opmanu
...
if {[form is_valid $form_name]} {
  # form valido dal punto di vista del templating system
    ...
    set cod_opmanu       [element::get_value $form_name cod_opmanu]
  ...
  # dopo l'inserimento posiziono la lista sul record inserito
    ...
    set link_gest      [export_url_vars cod_dimp last_cod_dimp nome_funz nome_funz_caller extra_par caller cod_impianto url_list_aimp url_aimp url_gage flag_no_link cod_opma data_ins]
    switch $funzione {
        M {set return_url   "coimdimp-gest-palm?funzione=V&$link_gest"}
        D { if {$flag_no_link == "F"} { 
                set return_url  "coimdimp-list-palm?$link_list"
	    } else {
		set return_url  "coimdimp-gest-palm?funzione=I&$link_gest"
	    }  
	}
        I {set return_url   "coimdimp-gest-palm?funzione=V&$link_gest"}
        V {set return_url   "coimdimp-list-palm?$link_list"}
    }
