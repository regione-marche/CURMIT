ad_page_contract {
    Add/Edit/Delete  form per la tabella "coimruol"
...
set link_gest [export_url_vars id_ruolo last_descrizione nome_funz nome_funz_caller extra_par caller]
...
if {[form is_valid $form_name]} {
  ...
  # controlli standard su numeri e date, per Ins ed Upd
    ...
    ||  $funzione == "M"
        ...
        if {[string equal $id_ruolo ""]} {
            element::set_error $form_name id_ruolo "Inserire Id ruolo"
            incr error_num
        }
	...
	if {$funzione == "M"} {
	    set where_cod "and id_ruolo <> :id_ruolo"
	} else {
	    set where_cod ""
	}

	if {[db_0or1row check_descrizione ""] == 1} {
    ...
    if {$funzione == "I"
    &&  $error_num == 0
    &&  [db_0or1row sel_ruol_check {}] == 1
    } {
      # controllo univocita'/protezione da double_click
        element::set_error $form_name id_ruolo "Il record che stai tentando di inserire &egrave; gi&agrave; esistente nel Data Base."
        incr error_num
    }
    ...
    switch $funzione {
        I {set dml_sql [db_map ins_ruol]}
        M {set dml_sql [db_map upd_ruol]}
        D {set dml_sql [db_map del_ruol]}
    }
  ...
  # dopo l'inserimento posiziono la lista sul record inserito
    ...
    set link_gest      [export_url_vars id_ruolo last_descrizione nome_funz nome_funz_caller extra_par caller]
    switch $funzione {
        M {set return_url   "coimruol-gest?funzione=V&$link_gest"}
        D {set return_url   "coimruol-list?$link_list"}
        I {set return_url   "coimruol-gest?funzione=V&$link_gest"}
        V {set return_url   "coimruol-list?$link_list"}
    }
