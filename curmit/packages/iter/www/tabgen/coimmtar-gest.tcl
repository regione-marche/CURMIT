...
set page_title      "Lista Aree associate al Manutentori"
set titolo          "Area del manutentore"
...
set link_list       [export_url_vars cod_manutentore]
...
if {[form is_request $form_name]} {
    ...
    set table_def [list \
	      ...
	      [list descrizione            "Descrizione areae" no_sort {l}] \
	      ]
	...
	"S" {# imposto la condizione per la pagina corrente
	    if {![string is space $curr_descrizione]} {
                set where_last " and upper(descrizione)   > upper(:curr_descrizione)"
	    } else {
                set where_last ""
	    }
	}
	...
	default {   # imposto la condizione per la prossima pagina
	    if {![string is space $last_descrizione]} {
	        set where_last " and upper(descrizione)   > upper(:last_descrizione)"
	    } else {
                set where_last ""
	    }
	}
    ...
    set table_result [ad_table -Tmax_rows $rows_per_page -Tmissing_text "Nessun dato corrisponde ai criteri impostati." -Textra_vars {cod_area cognome last_descrizione curr_descrizione flag_ins nome_funz nome_funz_caller url_list_area url_area extra_par} go $sel_mtar $table_def]
	...
	set last_descrizione [list  $cognome $cod_manutentore]
...
if {[form is_valid $form_name]} {
  # form valido dal punto di vista del templating system
    ...
    set lista_manu [array names relazione]     
    foreach cod_area $lista_manu {
            set dml_sql [db_map ins_mtar]
    ...
    ad_returnredirect coimmtar-list?[export_url_vars cod_manutentore last_descrizione curr_descrizione flag_ins nome_funz nome_funz_caller extra_par url_list_area url_area]
    ad_script_abort
}
