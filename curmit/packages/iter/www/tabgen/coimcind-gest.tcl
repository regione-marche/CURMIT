ad_page_contract {
    Add/Edit/Delete  form per la tabella "coimcind"
...
set link_gest      [export_url_vars cod_cind last_cod_cind nome_funz extra_par caller]

iter_set_func_class $funzione

# Personalizzo la pagina
set link_list_script {[export_url_vars last_cod_cind caller nome_funz]&[iter_set_url_vars $extra_par]}
set link_list        [subst $link_list_script]
set titolo           "Campagna dichiarazioni"
...
element create $form_name last_cod_cind -widget hidden -datatype text -optional
...
if {[form is_request $form_name]} {
    ...
    element set_properties $form_name last_cod_cind -value $last_cod_cind

    if {$funzione == "I"} {
        
    } else {
      # leggo riga
        if {[db_0or1row sel_cind ""] == 0
        } {
...
if {[form is_valid $form_name]} {
  ...
  # controlli standard su numeri e date, per Ins ed Upd
	...
	if {$funzione == "I"} {
	    ...
	    set where_cod " and cod_cind <> :cod_cind"
	}
	    ...
	    if {[db_0or1row sel_cind_count ""] == 0} {
		set conta 0
	    }
	    if {$conta > 0
	    && $stato == 1
	...
	if {[string equal $descrizione ""]} {
	    ...
	    switch $funzione {
		"I" {set where_cod ""}
		"M" {set where_cod "and cod_cind <> :cod_cind"}
	    }
	...
	if {[db_0or1row sel_cind_2 ""] == 0} {
	    set stato ""
	}

	if {$stato == "2"} {
	    element::set_error $form_name cod_cind "Cancallazione impossibile: campagna chiusa"
	    incr error_num
	}
    }
    ...
    switch $funzione {
	    ...
	    set dml_sql  [db_map ins_cind]
        }
        M {
	    set dml_sql  [db_map mod_cind] 
        }
        D {
	    set dml_sql  [db_map del_cind]
        }
    }
  ...
  # dopo l'inserimento posiziono la lista sul record inserito
    ...
    set link_gest      [export_url_vars cod_cind last_cod_cind nome_funz extra_par caller]
    switch $funzione {
        M {set return_url   "coimcind-gest?funzione=V&$link_gest"}
        D {set return_url   "coimcind-list?$link_list"}
        I {set return_url   "coimcind-gest?funzione=V&$link_gest"}
        V {set return_url   "coimcind-list?$link_list"}
    }
