ad_page_contract {
    Add/Edit/Delete  form per la tabella "coimtano"
    ...
    @cvs-id          coimtano-gest.tcl

    USER  DATA       MODIFICHE
    ===== ========== =========================================================================
    san02 10/10/2017 Aggiunto maxlength 200 al campo descr_tano 

    san01 22/03/2017 Aggiunto ASL alle options del campo ente_competente 
...
#san02 aggiunto maxlength 200
element create $form_name descr_tano \
    -label   "descr_tano" \
    -widget   textarea \
    -datatype text \
    -html    "cols 50 rows 4  maxlength 200 $readonly_fld {} class form_element" \
    -optional
...
element create $form_name flag_tipo_ispezione \
    -label   "flag_tipo_ispezione" \
    -widget   select \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
    -optional \
    -options {{{} {}} {{Formali} F}  {{Critici} C}  {{Annotazioni} A}}


element create $form_name ente_competente \
    -label   "ente_competente" \
    -widget   select \
    -datatype text \
    -html    "$disabled_fld {} class form_element" \
    -optional \
    -options {{{} {}} {{Regione} R}  {{Area Vasta} A}  {{Comune} C} {{INAIL} I} {{VVF} V}  {{ASL} S}}
...
if {[form is_request $form_name]} {
	...
	element set_properties $form_name flag_tipo_ispezione    -value $flag_tipo_ispezione
        element set_properties $form_name ente_competente    -value $ente_competente
...
if {[form is_valid $form_name]} {
    ...
    set flag_tipo_ispezione [element::get_value $form_name flag_tipo_ispezione]
    set ente_competente [element::get_value $form_name ente_competente]
	...
	||  $funzione == "M"
		...
		&& ( $flag_ente  != "P" &&  $sigla_prov != "LI")
		&& ( $flag_ente  != "C" && $sigla_prov != "RO") } {
    ...
    if {$funzione == "I" && $error_num == 0 && [db_0or1row sel_tano_check {}] == 1} {
    ...
    if {$funzione == "D" && $conta_anom > 0} {
	element::set_error $form_name cod_tano "Impossibile eliminare il record: sono presenti anomalie di questo tipo collegate a Mod. H o rapporti di ispezione"
        incr error_num
    }
    ...
    switch $funzione {
        I {set dml_sql [db_map ins_tano]}
        M {set dml_sql [db_map upd_tano]}
        D {set dml_sql [db_map del_tano]}
    }
    ...
    set link_gest      [export_url_vars cod_tano last_cod_tano nome_funz nome_funz_caller extra_par caller]
    switch $funzione {
        M {set return_url   "coimtano-gest?funzione=V&$link_gest"}
        D {set return_url   "coimtano-list?$link_list"}
        I {set return_url   "coimtano-gest?funzione=V&$link_gest"}
        V {set return_url   "coimtano-list?$link_list"}
    }
