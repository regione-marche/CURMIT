ad_proc iter_rgas_cari {
	...
	db_transaction {
	    ...
	    lappend head_cols "Tipologia impianto"
	    ...
	    lappend file_cols "inp_tipologia_imp"
		...
		set ws_tipologia_imp       [string trim    $inp_tipologia_imp]
		...
		if {$carica == "S"} {
		    if {![string equal $ws_tipologia_imp ""]} {
			set descr_tpim $ws_tipologia_imp
			db_1row sel_tpim_check ""
			# con la query precedente ho valorizzato ctr_tpim
			# e cod_tpim con il max(cod_tpim)
			if {$ctr_tpim == 0} {
			    set carica        "N"
			    set motivo_scarto "Tipologia impianto non esistente in tabella Tipologie"
			}
			if {$ctr_tpim > 1} {
			    set carica        "N"
			    set motivo_scarto "Trovate più tipologie con questa denominazione"
			}
		    }
		}
		...
		if {$carica == "S"} {
			...
			set ws_consumo_annuo 0
		    } else {
			if {![string is double $ws_consumo_annuo]} {
			    set carica        "N"
	...
	ns_log Error "iter_rgas_cari: $error_msg"
    }
    return
}
