...
#			set desc_errore "La potenza al focolare nominale impianto e' formalmente errata"
#			incr errori
#			db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'potenza_foc_nom' , :desc_errore )"
#		    }
#		    if {![string is double $pot_nom_foc_gen]} {
#			set desc_errore "La potenza al focolare nominale generatore e' formalmente errata"
#			incr errori
#			db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'pot_nom_foc_gen' , :desc_errore )"
#		    }

#		    if {![string is double $pot_nom_utile_gen]} {
#			set desc_errore "La potenza nominale utile generatore e' formalmente errata"
#			incr errori
#			db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'pot_nom_utile_gen' , :desc_errore )"
#		    }

#		    if {![string is double $pot_utile_nom_mod]} {
#			set desc_errore "La potenza utile nominale sul modello e' formalmente errata"
#			incr errori
#			db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'pot_utile_nom_mod' , :desc_errore )"
#		    }
...
#							db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'P' where id_riga = :id_riga"
			...
			if {$impiantotrovato == "s"} {
			    ...
			    if {[db_0or1row query "select coalesce(matricola,'?') as matricola_esistente,
                                                          coalesce(cod_cost,'?') as costruttore_esistente 
                                                     from coimgend 
                                                    where cod_impianto = :cod_impianto
                                                      and gen_prog = :gen_prog"]} {
				if {($matricola != $matricola_esistente) || ($cod_cost_chk != $costruttore_esistente)} {
				    set desc_errore "Il generatore $gen_prog esiste con matricola o costruttore diversi. Verificare. "
				    incr errori
				    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'matricola' , :desc_errore )"
				    db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'S' where id_riga = :id_riga"
				}
	...
	if {[string equal $err_log ""]} {
		...
		# Scrivo la procedura che controller�Á� l'esistenza o meno nel db dei cittadini
		# e ne curer�Á� l'eventuale inserimento
		set citt_control {
...
#					if {[db_0or1row sel_dimp_check_data_controllo ""] != 0} {
#					    set tariffa_reg "7"
...
#						    } else {
#								incr err_count
#								append msg_errore "Transazione non avvenuta correttamente, riprovare tra qualche minuto ($result(page))" 
#						    }
						}
...
#				    set gen_prog [db_string query "select coalesce(max(gen_prog)::integer + 1,1) from coimgend where cod_impianto = :cod_impianto" -default 1]
				    db_dml ins_gend ""
				...
				if {[db_0or1row sel_impianto "select a.cod_impianto from coimaimp a
                                                                         where a.cod_provincia = :cod_prov_chk
                                                                          ...
                                                                          and a.potenza       = :potenza_foc_nom
                                                                          and a.stato         = 'A'
                                                                          $where_indirizzo limit 1"] == 0} {

				    set potenza_utile_nom $pot_nom_utile_gen
...
#				ns_log notice "prova inserisce dimp cod_impianto = $cod_impianto gen_prog = $gen_prog"
