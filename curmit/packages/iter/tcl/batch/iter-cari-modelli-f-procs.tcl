...
# apro 1    
    ...
    ns_log Notice "Inizio della procedura iter-cari-modelli-f ..."
...
# apro 2    
	...
	if {![string equal $nome_tabella ""]} {
	    set nome_tabella_anom [string map {FCARI ANOM} $nome_tabella]
	...
	# dichiaro di scrivere in formato iso West European
	fconfigure $file_tot_id -encoding iso8859-1
	...
	set err_log ""
		...
		if {$lunghezza_letta == $lunghezza_attesa
			...
			if {$cod_manutentore != $cod_manutentore_chk
			    ...
			    append err_log "L'intero lotto e' stato scartato in quanto l'utente che effettua il caricamento non corrisponde alla ditta presente nel file"
			    break
			}
		    }
		} else {
		    append err_log "L'intero lotto e' stato scartato in quanto una o piu' righe hanno lunghezza diversa rispetto a quella prevista (lunghezza letta : $lunghezza_letta lunghezza  prevista $lunghezza_attesa)."
		    break
		}
		...
		if {[db_0or1row sel_cari_manu "select '1' from coimcari_manu where cod_manutentore = :cod_manutentore"]} {
		    append err_log "L'intero lotto e' stato scartato in quanto la ditta di manutenzione con codice $cod_manutentore sta gia' effettuando un caricamento su questo ente."
		}
	    }
	...
	if {[string equal $err_log ""]} {
		...
		while {$i < $count_fields} {
			...
			if {[string equal $obbligatorio "S"]} {
				...
				set desc_errore "Il campo e' obbligatorio. Valorizzare $valori_possibili."
				incr errori
		...
		if {$errori > 0} {
			...
			if {[string equal $cod_impianto_est ""]} {
			    set desc_errore "Manca  codice  impianto  nel  file"
			    incr errori
			...
			"A" { 
				...
				set desc_errore "Il responsabile (amministratore) non e' valorizzato"
				incr errori
				...
				if {[db_0or1row sel_citt_check "select cod_cittadino as cod_citt from coimcitt
		                                                         where upper(cognome) = :cognome_resp
                                                                           and cod_fiscale = :cod_fiscale_resp
	                                                                $where_nome_chk
	                                                                $where_indirizzo_chk
	                                                                $where_comune_chk
	                                                                $where_provincia_chk
                                                                 and cod_cittadino like 'AM%'"] == "0"} {
				    set desc_errore "Il responsabile (amministratore) non e' presente nell'anagrafica ricontrolla i dati relativi"
				    incr errori
				    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'cognome_resp' , :desc_errore )"
			...
			"I" {
				...
				set desc_errore "Il responsabile (intestatario) non e' valorizzato"
				incr errori
			...
			"O" {
				...
				&& [string equal $cognome_resp ""]} {
				set desc_errore "Il responsabile (occupante) non e' valorizzato"
				incr errori
			...
			"P" {
				...
				&& [string equal $cognome_resp ""]} {
				set desc_errore "Il responsabile (proprietario) non e' valorizzato"
				incr errori
			...
			"T" {
			    if {[string equal $cognome_resp ""]} {
				set desc_errore "Il responsabile (terzo) non e' valorizzato"
				incr errori
				db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'cognome_resp' , :desc_errore )"
			    } else {
				if {[string equal $nome_resp ""]} {
			...
			if {![info exist costruttori($cod_cost_bruc)]} {
			    ...
			    set cod_cost_bruc_chk costruttori($cod_cost_bruc)
			}
		    } else {
			set cod_cost_bruc_chk $cod_cost_chk
		    }
			...
			set desc_errore "La potenza deve essere superiore o uguale a 35 kW"
			incr errori
			...
			if {[db_0or1row sel_pot "select '1' from coimpote where potenza_min <= :potenza_foc_nom and potenza_max >= :potenza_foc_nom"] == 0} {
			    set desc_errore "La potenza non e' compresa in nessuna fascia."
			    incr errori
			    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'potenza_foc_nom' , :desc_errore )"
			}
		    }
			...
			set desc_errore "La potenza utile del generatore e' diversa da quella relativa al modello ($pot_utile_nom_mod)."
			incr errori
			...
			if {![string equal $cod_impianto_est ""]} {
				...
				if {[db_0or1row sel_aimp_check "select cod_impianto from coimaimp where 1 = 1 $where_indirizzo and cod_impianto_est = :cod_impianto_est"]} {
				    ...
				    if {[db_0or1row sel_generatore "select b.gen_prog as gen_prog_check 
	                                                            ...
	                                                            where a.cod_impianto     = b.cod_impianto
	                                                              and a.cod_impianto_est = :cod_impianto_est
                                                                      and b.gen_prog         = :gen_prog limit 1"]} {
					if {[db_0or1row sel_generatore "select b.gen_prog as gen_prog_check from coimaimp a
	                                                                               , coimgend b
	                                                                         where a.cod_impianto = b.cod_impianto
	                                                                           and a.cod_impianto_est = :cod_impianto_est
	                                                                           ...
	                                                                           and b.cod_cost  = :cod_cost_chk 
                                                                                   and b.gen_prog = :gen_prog limit 1"]} {
					    db_dml upd_riga "update $nome_tabella set gen_prog_catasto = :gen_prog_check 
                                                              where id_riga = :id_riga"
					} else {
					    set desc_errore "Numero generatore corretto ma matricola o costruttore non corrispondono a quelli in catasto."
					    incr errori
					    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'cod_impianto_est' , :desc_errore )"
					    db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'S' where id_riga = :id_riga"
				    ...
				    set desc_errore "Codice impianto trovato ma con indirizzo diverso."
				    incr errori
				    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'indirizzo' , :desc_errore )"
				    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'toponimo' , null )"
				    db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'S' where id_riga = :id_riga"
				...
				set desc_errore "Codice impianto non trovato nel comune indicato."
				incr errori
				db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'cod_impianto_est' , :desc_errore )"
				db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'S' where id_riga = :id_riga"
			    }
			} else {
			    set desc_errore "Manca il codice impianto nel file."
			    incr errori
			    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'cod_impianto_est' , :desc_errore )"
			    db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'S' where id_riga = :id_riga"
			}		    
		...
		if {$errori == 0
			...
			if {[db_0or1row sel_dimp "select cod_dimp from coimdimp where cod_impianto = :cod_impianto and data_controllo = :data_controllo and gen_prog = :gen_prog"]} {
			    set desc_errore "Dichiarazione gia' presente controlla la data"
			    incr errori
	...
	if {[string equal $err_log ""]} {
		...
		# Scrivo la procedura che controllre l'esistenza o meno nel db dei cittadini
		...
		db_foreach sel_righe_buone "select * from $nome_tabella where flag_stato is null" {
			...
			if {$flag_portafoglio == "T"
			    ...
			    if {![string equal $cod_impianto_est ""]} {
				db_1row query "select potenza as potenza_foc_nom, numero as civico 
                                                   from coimaimp 
			    ...
			    if {[db_0or1row esistenza__dimp "select 1 
                                                             ...
                                                             where a.cod_impianto   = b.cod_impianto
                                                               ...
                                                               and coalesce(a.numero,' ') = coalesce(:civico,' ')
				...
				if {$pot_focolare_nom_check == "0.00"
				    ...
				    incr err_count
				    append msg_errore "Non e' stato possibile calcolare l'importo del contributo regionale in quanto la potenza e' errata o 0,00 ($potenza_foc_nom)" 
				} else {
				    ...
				    ns_log notice "httpget balance: risultato $risultato dati $data"
					...
					array set result $data
					set risultato [string range $result(page) 0 [expr [string first " " $result(page)] - 1]]
					ns_log notice "httpget itermove: risultato $risultato dati $data"
					if {$risultato == "OK"} {
			...
			if {$err_count == 0} {
			    ...
			    set potenza_focolare_nom_tot [db_string sel_pot_tot "select sum(pot_focolare_nom) from coimgend where cod_impianto = :cod_impianto" -default 0]
			    set potenza_utile_nom [db_string sel_pot_tot "select sum(pot_utile_nom) from coimgend where cod_impianto = :cod_impianto" -default 0]
			    set num_tot_gen [db_string sel_gen_tot "select count(*) from coimgend where cod_impianto = :cod_impianto" -default 0]
			...
			if {![string equal $cod_impianto_catasto ""]
			    ...
			    set msg_errore "Il generatore e' da rottamare ma non e' stato trovato nel database."
			    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'cod_impianto_est' , :msg_errore )"
			    db_dml upd_riga "update $nome_tabella set numero_anomalie = '1' , flag_stato = 'S' where id_riga = :id_riga"
			}
		    }
	...
	ns_log Error "iter-cari-modelli-f: $error_msg"
    }
    
    ns_log Notice "Fine della procedura iter-cari-modelli-f"
