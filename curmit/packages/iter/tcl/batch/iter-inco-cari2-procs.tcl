...
#codice istat al momento non obbligatorio
		 ...
		 if {![string equal $ws_cod_istat ""]} {
			...
			set cod_istat [string range $ws_cod_istat 3 7]
		...
		if {$carica == "S"} {
			...
			switch $ws_combustibile {
			    ...
			    "NAFTA"               {set cod_combustibile "1"}
			    "COMBUSTIBILE SOLIDO" {set cod_combustibile "12"}
			    "LEGNA"               {set cod_combustibile "6"}
			    "TELERISCALDAMENTO"   {set cod_combustibile "7"}
...
#unità di misura non obbligatoria
...
#Tipo contratto non  obbligatorio
