ad_proc iter-scar-manu {
	...
	set criteri_selezione "Selezione effettuata alle [iter_set_systime] del [iter_edit_date [iter_set_sysdate]] di tutti i modelli"
	...
	iter_put_csv $file_id criteri_selezione_list
       iter_put_csv $file_id file_cols
	...
	db_foreach sel_dimp_aimp "" {
		...
		lappend file_col_list [string map { "\r" " " "\n" " " } [set $column_name]]
	    }
	    iter_put_csv $file_id file_col_list ;
	    
	} if_no_rows {
	    set msg_err      "Nessun modello  selezionato con i criteri utilizzati"
	...
	set esit_list     ""
        set esit_riga     [list "Scarico dichiarazioni" $file_url $file_path]
