...
<queryset>
    ...
    <fullquery name="iter_car_aces.sel_dist">
       <querytext>
                 select ragione_01||' '||nvl(ragione_02, '') as nome_dist
                   from coimdist
    ...
    <partialquery name="iter_car_aces.set_where_cognome_nome">
       <querytext>
       and trim(trim(nvl(cognome,''))||' '||trim(nvl(nome,''))) =
           trim(trim(nvl(:cognome,''))||' '||trim(nvl(:nome,'')))
       </querytext>
    </partialquery>
    ...
    <fullquery name="iter_car_aces.sel_aces_2">
       <querytext>
              select coimaces_s.nextval as cod_aces
                from dual
       </querytext>
    </fullquery>
