ad_proc iter_inco_cari_TN {
	...
	db_transaction {
	    ...
	    lappend file_cols "inp_cod_utenza"
	    lappend file_cols "inp_tariffa"
	    lappend file_cols "inp_consumo_annuo"
	    lappend file_cols "inp_comune"
	    lappend file_cols "inp_localita"
	    lappend file_cols "inp_via"
	    lappend file_cols "inp_civico"
	    lappend file_cols "inp_esponente"
	    ...
	    # Salto i primi 9 record che deve essere di testata
		...
		set ws_cod_utenza          [string trim    $inp_cod_utenza]
		set ws_tariffa             [string trim    $inp_tariffa]
		set ws_consumo_annuo       [string trim    $inp_consumo_annuo]
		set ws_comune              [string trim    $inp_comune]
		set ws_localita            [string trim    $inp_localita]
		set ws_via                 [string trim    $inp_via]
		set ws_civico              [string trim    $inp_civico]
		set ws_civico              [string trimleft $ws_civico "0"]
		set ws_esponente           [string trim    $inp_esponente]

		# controllo record
		set carica "S"

		switch $ws_tariffa {
		    "Riscald. superiore 100 kW" { set ws_tariffa "03" }
		    "Riscald.autonomo e acqua calda" { set ws_tariffa "04"  }
		    "Riscald. centralizzato" { set ws_tariffa "05" }
		    "Riscald.central.piccoli condomini" { set ws_tariffa "07" }
		    default { set ws_tariffa "" }
		}
		...
		if {$carica == "S"} {
		    if {[string equal $ws_via ""]} {
			set carica        "N"
			set motivo_scarto "Via non valorizzato"
		    }
		}
		...
		if {$carica == "S"} {
			...
			if {$ws_consumo_annuo == "Error"} {
			    set carica        "N"
			    set motivo_scarto "Consumo annuo deve essere numerico"
			}
		    }
		}
		...
		&&  $carica == "S"
		    ...
		    set pos [string first " " $ws_via]
		    set descr_topo  [string range $ws_via 0 [expr $pos -1]]
		    set descrizione [string range $ws_via [expr $pos +1] end]
		...
		if {$carica == "S"} {
		    ...
		    if {[string length $ws_civico] > 8} {
			set numero        [string range $ws_civico 0 7]
		    }
		    ...
		    if {[string length $ws_civico] <= 8} {
			set numero         $ws_civico
		    } else {
			set numero         [string range $ws_civico 0 7]
		    }
		    set esponente          $ws_esponente
