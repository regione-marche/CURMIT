ad_proc iter_cari_modelli {
    ...
    @author          dob (ereditato suo malgrado)
...
# bollino
		...
		# fine bollini
		    ...
		    #controllo sulla potenza
 		    ...
 		    if {$potenza_foc_nom < 4 || $potenza_foc_nom >= 35} {
			set desc_errore "La potenza al focolare nominale deve essere compresa tra 4 e 34.99 kW"
			incr errori
			...
			if {![string equal $cod_impianto_est ""]} {
				...
				if {[db_0or1row sel_aimp_check "select cod_impianto 
				    ...
				    if {[db_0or1row sel_generatore "select b.matricola as matricola_esistente,
					...
					if {($matricola != $matricola_esistente && "" != $matricola_esistente) || ($cod_cost_chk != $costruttore_esistente && "" != $costruttore_esistente)} {
					    ...
					    set desc_errore "Codice impianto trovato ma il generatore ha costruttore o matricola diversa"
					    incr errori
					    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'marca o matricola' , :desc_errore )"
					    db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'S' where id_riga = :id_riga"
					} 						    
				    } else {
					set desc_errore "Codice impianto corretto, ma il numero generatore non corrisponde a quello in catasto."
					incr errori
					db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'marca o matricola' , :desc_errore )"
					db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'S' where id_riga = :id_riga"
				    }
				    ...
				    set desc_errore "Codice impianto presente nel comune, ma con indirizzo diverso."
				    incr errori
				    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'indirizzo' , :desc_errore )"
				    db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'toponimo' , null )"
				    db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'S' where id_riga = :id_riga"
				...
				set desc_errore "Non esiste il codice impianto nel comune indicato."
				incr errori
				db_dml ins_anom "insert into $nome_tabella_anom values ( :id_riga , :cod_manutentore, 'codice impianto' , :desc_errore )"
				db_dml upd_riga "update $nome_tabella set numero_anomalie = :errori , flag_stato = 'S' where id_riga = :id_riga"
			    }
			} else {
			    set desc_errore "Manca codice impianto nel file"
