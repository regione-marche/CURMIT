ad_proc iter_cari_manu_new {
    ...
    sim02            istat piu' un progressivo
    ...
    ns_log Notice "Inizio della procedura iter_cari_manu"
	...
	set controllo_inserimento_aimp {
	    ...
	    if {![string equal $cod_via_impianto ""]
	     && ![string equal $cod_comune_impianto ""]
	     && [db_0or1row sel_cod_viae "select '1' from coimviae where cod_via = :cod_via_impianto and cod_comune = :cod_comune_impianto"] == 1} {
		set cod_via $cod_via_impianto
		...
		# preparo i controlli per vedere se l'impianto senza codice esiste nel database
		# altrimenti verrà scartato
		...
		if {![string equal $cod_cost_bruc ""]} {
		    if {[db_0or1row sel_cost_b "select '1' from coimcost where upper(descr_cost) = upper(:cod_cost_bruc) limit 1"] == 0} {
			incr err_count
			append msg_errore "L'impianto è privo di codice: marca bruciatore inesistente nel database"
		    } else {
			set cod_cost_bruc [db_string sel_cost "select cod_cost as cod_cost_bruc from coimcost where upper(descr_cost) = upper(:cod_cost_bruc) limit 1"]
		    }
		}
		if {![string equal $cod_opmanu ""]} {
		    if {[db_0or1row sel_opma "select '1' from coimopma where cod_opma = upper(:cod_opmanu)"] == 0} {
			incr err_count
			append msg_errore "L'impianto è privo di codice: operatore manut. inesistente nel database"
		    }
		}
		
		set matricola_chk $matricola
		set modello_chk   $modello
		if {[db_0or1row sel_cost "select '1' from coimcost where upper(descr_cost) = upper(:marca) limit 1"] == 0} {
		    incr err_count
		    append msg_errore "L'impianto è privo di codice: marca inesistente nel database"
		} else {
		...
		if {$err_count == 0} {
			...
			if {$flag_new_aimp == "T"} {
				...
				if {$coimtgen(ente) eq "CPESARO" || $coimtgen(ente) eq "PPU" || $coimtgen(ente) eq "CFANO" || $coimtgen(ente) eq "CANCONA" || $coimtgen(ente) eq "PAN" || $coimtgen(ente) eq "CJESI" || $coimtgen(ente) eq "CSENIGALLIA"} {#sim01: aggiunta if ed il suo contenuto
					...
					} elseif {$coimtgen(ente) eq "PTA"} {#sim02 if e suo contenuto
				...
				db_dml upd_resp "update coimaimp set cod_responsabile = :cod_citt where cod_impianto_est = :cod_impianto_est"
			    }
			}
		    }
		}
		#codice impianto esterno presente
	    } else {
		if {[db_0or1row sel_cost "select '1' from coimcost where upper(descr_cost) = upper(:marca) limit 1"] == 0} {
		    incr err_count
		    append msg_errore "Marca inesistente nel database"
		} else {
		    set cod_cost [db_string sel_cost "select cod_cost as cod_cost from coimcost where upper(descr_cost) = upper(:marca) limit 1"]
		}

		set where_cod " and a.cod_impianto_est = :cod_impianto_est"
		...
		if {[db_0or1row sel_aimp_check ""] == 0} {
			...
			if {[db_0or1row sel_cost_b "select '1' from coimcost where upper(descr_cost) = upper(:cod_cost_bruc) limit 1"] == 0} {
			    incr err_count
			    append msg_errore "Marca bruciatore inesistente nel database"
			} else {
			    set cod_cost_bruc [db_string sel_cost "select cod_cost as cod_cost_bruc from coimcost where upper(descr_cost) = upper(:cod_cost_bruc) limit 1"]
			}
		    }
			...
			if {[db_0or1row sel_opma "select '1' from coimopma where cod_opma = upper(:cod_opmanu)"] == 0} {
			    incr err_count
			    append msg_errore "Operatore manut. inesistente nel database"
			}
		    }
			...
			if {![string equal $cod_impianto_est ""]} {
			    set where_codi "where cod_impianto_est = :cod_impianto_est"
			} else {
			    set where_codi "where cod_impianto = :cod_impianto"
			}
	...
	set inserimento_dimp {
		...
		set data_controllo [iter_edit_date $data_controllo]
		set data_controllo [iter_check_date $data_controllo]
		set data_controllo [db_string sel_dat "select to_date(:data_controllo, 'yyyymmdd')"]
		if {$err_count == 0} {
		    if {$flag_portafoglio == "T"
			&& $data_controllo >= "2008-08-01" } {
			...
			if {$data_controllo >= $data_insta_check} {
				...
				if {$pot_focolare_nom_check == "0.00"
				    ...
				    incr err_count
				    append msg_errore "Non è stato possibile calcolare l'importo del contributo regionale in quanto la potenza è 0,00" 
				} else {
		...
		append msg_errore "Dichiarazione già presente nel database (verificare data controllo e progressivo generatore); "
		# Scrivo la riga sull'output del csv da scaricare
		iter_put_csv $file_err_id file_inp_col_list |
		# Incremento il conteggio degli modelli non accettati
		incr line_not_ok
	...
	lappend head_cols "cod. provincia impianto"
	lappend file_cols "cod_provincia_impianto"
	lappend head_cols "cod. comune impianto"
	lappend file_cols "cod_comune_impianto"
	lappend head_cols "cod. via impianto"
	lappend file_cols "cod_via_impianto"
