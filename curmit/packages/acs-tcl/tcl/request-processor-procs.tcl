ad_library {
    ...
    @cvs-id $Id: request-processor-procs.tcl,v 1.113.2.5 2013/09/19 10:13:41 gustafn Exp $
}
...
ad_proc rp_report_error {
    ...
    #sim il messaggio dato a video deve essere generico per motivi di sicurezza
    set message_page "Il messaggio di errore è stato scritto all'interno del file di log";#sim
        
    #Serve the stacktrace
    #sim messo message_page al posto di message    
    set params [list [list stacktrace $message_page] [list user_id $user_id] [list error_file $error_file] [list prev_url $prev_url] [list feedback_id $feedback_id] [list error_url $error_url] [list bug_package_id $bug_package_id] [list vars_to_export $vars_to_export]]
	...
	#sim il messaggio dato a video deve essere generico per motivi di sicurezza
	set error_message_page "Il messaggio di errore è stato scritto all'interno del file di log";#sim
        set rendered_page "</table></table></table></h1></b></i><blockquote><pre>[ns_quotehtml $error_message_page]</pre></blockquote>[ad_footer]"
    }
