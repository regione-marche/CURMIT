...
TRAILING NULLCOLS
(COD_DIMP                      CHAR(8) NULLIF (COD_DIMP = BLANKS) "RTRIM(:COD_DIMP)"
...
,OSSERVAZIONI                  CHAR(4000) NULLIF (OSSERVAZIONI = BLANKS) "RTRIM(:OSSERVAZIONI)"
,RACCOMANDAZIONI               CHAR(4000) NULLIF (RACCOMANDAZIONI = BLANKS) "RTRIM(:RACCOMANDAZIONI)"
,PRESCRIZIONI                  CHAR(4000) NULLIF (PRESCRIZIONI = BLANKS) "RTRIM(:PRESCRIZIONI)"
...
,DELEGA_RESP                   CHAR(50) NULLIF (DELEGA_RESP = BLANKS) "RTRIM(:DELEGA_RESP)"
,DELEGA_MANUT                  CHAR(50) NULLIF (DELEGA_MANUT = BLANKS) "RTRIM(:DELEGA_MANUT)"
,NUM_BOLLO                     CHAR(20) NULLIF (NUM_BOLLO = BLANKS) "RTRIM(:NUM_BOLLO)"
...
,COD_DOCU_DISTINTA             CHAR(8) NULLIF (COD_DOCU_DISTINTA = BLANKS) "RTRIM(:COD_DOCU_DISTINTA)"
,SCAR_CAN_FU                   CHAR(1) NULLIF (SCAR_CAN_FU = BLANKS)
,TIRAGGIO                      DECIMAL EXTERNAL(11) NULLIF (TIRAGGIO = BLANKS)
,ORA_INIZIO                    CHAR(8) NULLIF (ORA_INIZIO = BLANKS) "RTRIM(:ORA_INIZIO)"
,ORA_FINE                      CHAR(8) NULLIF (ORA_FINE = BLANKS) "RTRIM(:ORA_FINE)"
,RAPP_CONTR                    CHAR(1) NULLIF (RAPP_CONTR = BLANKS)
,RAPP_CONTR_NOTE               CHAR(4000) NULLIF (RAPP_CONTR_NOTE = BLANKS) "RTRIM(:RAPP_CONTR_NOTE)"
,CERTIFICAZ                    CHAR(1) NULLIF (CERTIFICAZ = BLANKS)
,CERTIFICAZ_NOTE               CHAR(4000) NULLIF (CERTIFICAZ_NOTE = BLANKS) "RTRIM(:CERTIFICAZ_NOTE)"
,DICH_CONF                     CHAR(1) NULLIF (DICH_CONF = BLANKS)
,DICH_CONF_NOTE                CHAR(4000) NULLIF (DICH_CONF_NOTE = BLANKS) "RTRIM(:DICH_CONF_NOTE)"
,LIBRETTO_BRUC                 CHAR(1) NULLIF (LIBRETTO_BRUC = BLANKS)
,LIBRETTO_BRUC_NOTE            CHAR(4000) NULLIF (LIBRETTO_BRUC_NOTE = BLANKS) "RTRIM(:LIBRETTO_BRUC_NOTE)"
,PREV_INCENDI                  CHAR(1) NULLIF (PREV_INCENDI = BLANKS)
,PREV_INCENDI_NOTE             CHAR(4000) NULLIF (PREV_INCENDI_NOTE = BLANKS) "RTRIM(:PREV_INCENDI_NOTE)"
,LIB_IMPIANTO_NOTE             CHAR(4000) NULLIF (LIB_IMPIANTO_NOTE = BLANKS) "RTRIM(:LIB_IMPIANTO_NOTE)"
,ISPESL                        CHAR(1) NULLIF (ISPESL = BLANKS)
,ISPESL_NOTE                   CHAR(4000) NULLIF (ISPESL_NOTE = BLANKS) "RTRIM(:ISPESL_NOTE)"
,DATA_SCADENZA                 DATE(10) "YYYY-MM-DD" NULLIF (DATA_SCADENZA = BLANKS)
,NUM_AUTOCERT                  CHAR(20) NULLIF (NUM_AUTOCERT = BLANKS) "RTRIM(:NUM_AUTOCERT)"
,ESAME_VIS_L_ELET              CHAR(1) NULLIF (ESAME_VIS_L_ELET = BLANKS)
,FUNZ_CORR_BRUC                CHAR(1) NULLIF (FUNZ_CORR_BRUC = BLANKS)
,LIB_USO_MAN_NOTE              CHAR(4000) NULLIF (LIB_USO_MAN_NOTE = BLANKS) "RTRIM(:LIB_USO_MAN_NOTE)"
