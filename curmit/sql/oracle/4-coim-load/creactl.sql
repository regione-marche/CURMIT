...
SELECT 'C1'
      ...
      ,RPAD(DECODE(column_id ,1 ,'('
                                ,','
                  ) || UPPER(decode(column_name,'DATA','"DATA"',column_name))
           ,31)
    || DECODE(data_type ,'CHAR'     ,'CHAR(' || data_length || ')'
                        ...
                        ,'FLOAT'    , 'DECIMAL EXTERNAL(' 
                                   || TO_CHAR(&dflt_precision+2) || ')'
                        ,'ERROR--' || data_type
             )
    || ' NULLIF (' || UPPER(decode(column_name,'DATA','"DATA"',column_name)) || ' = BLANKS)'
    || DECODE(data_type ,'VARCHAR'  , ' "RTRIM(:'||UPPER(decode(column_name,'DATA','"DATA"',column_name))||')"'
                        ,'VARCHAR2' , ' "RTRIM(:'||UPPER(decode(column_name,'DATA','"DATA"',column_name))||')"')
